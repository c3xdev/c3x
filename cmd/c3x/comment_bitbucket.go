...
func commentBitbucketCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "bitbucket",
		Short: "Post a C3X comment to Bitbucket",
		Long:  "Post a C3X comment to Bitbucket",
		Example: `  Update comment on a pull request:

      c3x comment bitbucket --repo my-org/my-repo --pull-request 3 --path c3x.json --bitbucket-token $BITBUCKET_TOKEN

  Post a new comment to a commit:

      c3x comment bitbucket --repo my-org/my-repo --commit 2ca7182 --path c3x.json --behavior delete-and-new --bitbucket-token $BITBUCKET_TOKEN`,
		ValidArgs: []string{"--", "-"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			commentOut, commentErr := buildCommentOutput(cmd, ctx, paths, render.MarkdownOptions{
			...
			if !dryRun {
				...
				err = pricingClient.AddEvent("c3x-comment", ctx.EventEnv())
				if err != nil {
  ...
  delete-and-new    Delete previous matching comments and create a new comment`)
	...
	cmd.Flags().String("tag", "", "Customize special text used to detect comments posted by C3X (placed at the bottom of a comment)")
