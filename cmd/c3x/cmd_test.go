...
import (
	...
	main "github.com/c3xdev/c3x/cmd/c3x"
...
var (
	...
	urlRegex         = regexp.MustCompile(`https://dashboard.c3x.dev/share/.*`)
	projectPathRegex = regexp.MustCompile(`(Project:) .*/(examples|cmd/c3x)/(.*)`)
	versionRegex     = regexp.MustCompile(`C3X (v|preview).*`)
...
func GoldenFileCommandTest(t *testing.T, testName string, args []string, testOptions *GoldenFileOptions, ctxOptions ...func(ctx *settings.Session)) {
...
func goldenFileCommandTest(t *testing.T, testName string, args []string, testOptions *GoldenFileOptions, hcl bool, ctxOptions ...func(ctx *settings.Session)) {
...
func GetCommandOutput(t *testing.T, args []string, testOptions *GoldenFileOptions, ctxOptions ...func(ctx *settings.Session)) []byte {
	t.Helper()
	...
	os.Setenv("C3X_VCS_REPOSITORY_URL", "https://github.com/c3xdev/c3x")
	os.Setenv("C3X_VCS_PULL_REQUEST_URL", "NOT_APPLICABLE")
	...
	main.Run(func(c *settings.Session) {
		enableCloud := false
...
func stripDynamicValues(actual []byte) []byte {
	...
	actual = urlRegex.ReplaceAll(actual, []byte("https://dashboard.c3x.dev/share/REPLACED_SHARE_CODE"))
	actual = projectPathRegex.ReplaceAll(actual, []byte("$1 REPLACED_PROJECT_PATH/$3"))
	actual = versionRegex.ReplaceAll(actual, []byte("C3X vREPLACED_VERSION"))
