...
func outputCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "output",
		Short: "Combine and output C3X JSON files in different formats",
		Long:  "Combine and output C3X JSON files in different formats",
		Example: `  Show a breakdown from multiple C3X JSON files:

      c3x output --path out1.json --path out2.json --path out3.json

  Create HTML report from multiple C3X JSON files:

      c3x output --format html --path "out*.json" --out-file render.html # glob needs quotes

  Merge multiple C3X JSON files:

      c3x output --format json --path "out*.json" # glob needs quotes

  Create markdown report to post in a GitHub comment:

      c3x output --format github-comment --path "out*.json" # glob needs quotes

  Create markdown report to post in a GitLab comment:

      c3x output --format gitlab-comment --path "out*.json" # glob needs quotes

  Create markdown report to post in a Azure DevOps Repos comment:

      c3x output --format azure-repos-comment --path "out*.json" # glob needs quotes

  Create markdown report to post in a Bitbucket comment:

      c3x output --format bitbucket-comment --path "out*.json" # glob needs quotes`,
		ValidArgs: []string{"--", "-"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			combined.Metadata.C3XCommand = "output"
			...
			b, err := render.FormatOutput(format, combined, opts)
			if err != nil {
				return err
			}

			pricingClient := apiclient.GetPricingAPIClient(ctx)
			err = pricingClient.AddEvent("c3x-output", ctx.EventEnv())
			if err != nil {
	...
	cmd.Flags().StringArrayP("path", "p", []string{}, "Path to C3X JSON files, glob patterns need quotes")
...
func shareCombinedRun(ctx *settings.Session, combined render.Report, inputs []render.ReportInput) apiclient.AddRunResponse {
