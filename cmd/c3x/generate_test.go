...
func TestGenerateConfig(t *testing.T) {
	...
	GoldenFileCommandTest(
		...
		[]string{
			...
			"config",
			"--template-path",
			path.Join("./testdata", dir, "c3x.yml.tmpl"),
			"--repo-path",
...
// in a tmp directory. It will then run the `c3x generate config` command
...
func TestGenerateConfigAutoDetect(t *testing.T) {
	...
	for _, dir := range dirs {
		...
		t.Run(name, func(tt *testing.T) {
			...
			templatePath := path.Join(dir, "c3x.yml.tmpl")
...
func assertGoldenFileTree(tt *testing.T, args []string, dir string) {
	...
	actual, err := os.ReadFile(path.Join(dir, "c3x-tree.txt"))
...
func assertGoldenFile(tt *testing.T, args []string, dir string) {
	...
	actual := GetCommandOutput(tt, args, nil, func(ctx *settings.Session) {
