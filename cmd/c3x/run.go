...
func addRunFlags(cmd *cobra.Command) {
	...
	cmd.Flags().String("config-file", "", "Path to C3X config file. Cannot be used with path, terraform* or usage-file flags")
	cmd.Flags().String("usage-file", "", "Path to C3X usage file that specifies values for usage-based resources")

	cmd.Flags().String("project-name", "", "Name of project in the render. Defaults to path or git repo name")
...
func runMain(cmd *cobra.Command, runCtx *settings.Session) error {
	...
	projectContexts := make([]*settings.ProjectSession, 0)
	...
	// Apply what-if overrides before cost calculation
	whatIfOverrides, _ := cmd.Flags().GetStringArray("what-if")
	if len(whatIfOverrides) > 0 {
		overrides := make([]*whatif.Override, 0, len(whatIfOverrides))
		for _, s := range whatIfOverrides {
			o, err := whatif.ParseOverride(s)
			if err != nil {
				return err
			}
			overrides = append(overrides, o)
		}
		applied, err := whatif.ApplyOverrides(projects, overrides)
		if err != nil {
			return err
		}
		if applied > 0 {
			logging.Logger.Info().Msgf("Applied %d what-if override(s)", applied)
		}
	}

	r, err := render.ToOutputFormat(runCtx.Config, projects)
	if err != nil {
		return err
	}

	if pr.prior != nil {
		r, err = render.CompareTo(runCtx.Config, r, *pr.prior)
		if err != nil {
			return err
		}
		runCtx.VCSMetadata.BaseCommit = vcs.Commit{
	...
	r.Metadata = render.NewMetadata(runCtx)
	...
	if runCtx.IsCloudUploadExplicitlyEnabled() {
		...
		if err != nil {
			logging.Logger.Err(err).Msg("Failed to upload to C3X Cloud")
		}

		r.RunID, r.ShareURL, r.CloudURL = result.RunID, result.ShareURL, result.CloudURL
	} else {
		logging.Logger.Debug().Msg("Skipping sending project results since C3X Cloud upload is not enabled.")
	}
	...
	b, err := render.FormatOutput(format, r, render.Options{
	...
	err = pricingClient.AddEvent("c3x-run", env)
	if err != nil {
		logging.Logger.Debug().Msgf("Error reporting event: %s", err)
	}
	...
	// Show inline recommendations if --recommend flag is set
	showRecommend, _ := cmd.Flags().GetBool("recommend")
	if showRecommend {
		result := recommend.Analyze(projects)
		if len(result.Recommendations) > 0 {
			cmd.Print(recommend.FormatTable(result, runCtx.Config.NoColor))
		}
	}

	if err := checkBudget(cmd, r); err != nil {
		return err
	}
...
type parallelRunner struct {
	cmd            *cobra.Command
	runCtx         *settings.Session
	pathMuxs       map[string]*sync.Mutex
	prior          *render.Report
	parallelism    int
	pricingFetcher *pricing.RateLookup
}

func newParallelRunner(cmd *cobra.Command, runCtx *settings.Session) (*parallelRunner, error) {
	...
	var prior *render.Report
	if runCtx.Config.CompareTo != "" {
		snapshot, err := render.Load(runCtx.Config.CompareTo)
		if err != nil {
	...
	return &parallelRunner{
		...
		pathMuxs:       pathMuxs,
		prior:          prior,
		pricingFetcher: pricing.NewPriceFetcher(runCtx, false),
	}, nil
}
...
func (r *parallelRunner) run() ([]projectResult, error) {
	...
	for _, p := range r.runCtx.Config.Projects {
		detectionOutput, err := cloud.Detect(r.runCtx, p, r.prior == nil)
		if err != nil {
			m := fmt.Sprintf("%s\n\n", err)
			m += fmt.Sprintf("  Try adding a config-file to configure how C3X should run. See %s for details and examples.", ui.LinkString("https://c3x.dev/config-file"))

			queue = append(queue, projectJob{index: i, err: engine.NewEmptyPathTypeError(errors.New(m)), ctx: settings.NewProjectContext(r.runCtx, p, map[string]interface{}{})})
			i++
			continue
		}
...
func (r *parallelRunner) runProvider(job projectJob) (out *projectOutput, err error) {
	...
	if r.cmd.Name() == "diff" && job.provider.Type() == "terraform_state_json" {
		m := "Cannot use Terraform state JSON with the c3x diff command.\n\n"
		m += fmt.Sprintf("Use the %s flag to specify the path to one of the following:\n", ui.PrimaryString("--path"))
		m += fmt.Sprintf(" - Terraform/Terragrunt directory\n - Terraform plan JSON file, see %s for how to generate this.", ui.SecondaryLinkString("https://c3x.dev/troubleshoot"))
		return nil, clierror.NewCLIError(errors.New(m), "Cannot use Terraform state JSON with the c3x diff command")
	}
		...
		if err = r.pricingFetcher.PopulatePrices(project); err != nil {
			...
			if errors.As(err, &apiErr) {
				...
				case apiclient.ErrorCodeExceededQuota:
					return nil, engine.NewDiagRunQuotaExceeded(apiErr)
				case apiclient.ErrorCodeAPIKeyInvalid:
					return nil, fmt.Errorf("%v\n%s %s %s %s %s\n%s %s.\n%s %s %s",
						apiErr.Msg,
						"Please check your",
						ui.PrimaryString(settings.CredentialsFilePath()),
						"file or",
						...
						"See",
						ui.PrimaryString("https://c3x.dev/support"),
...
func (r *parallelRunner) uploadCloudResourceIDs(projects []*engine.Workspace) error {
	...
	logging.Logger.Debug().Msg("Sending resource IDs to C3X Cloud for usage estimates")

	for _, project := range projects {
		if err := pricing.UploadCloudResourceIDs(r.runCtx, project); err != nil {
...
func (r *parallelRunner) hasCloudResourceIDToUpload(projects []*engine.Workspace) bool {
	for _, project := range projects {
...
func (r *parallelRunner) buildResources(projects []*engine.Workspace) {
	var projectPtrToUsageMap map[*engine.Workspace]engine.ConsumptionMap
	if r.runCtx.Config.UsageAPIEndpoint != "" {
		projectPtrToUsageMap = r.fetchProjectUsage(projects)
	}

	engine.BuildEstimates(projects, projectPtrToUsageMap)
}

func (r *parallelRunner) fetchProjectUsage(projects []*engine.Workspace) map[*engine.Workspace]engine.ConsumptionMap {
	coreResourceCount := 0
	for _, project := range projects {
	...
	logging.Logger.Debug().Msgf("Retrieving usage defaults for %s from C3X Cloud", resourceStr)

	projectPtrToUsageMap := make(map[*engine.Workspace]engine.ConsumptionMap, len(projects))

	for _, project := range projects {
		usageMap, err := pricing.FetchUsageData(r.runCtx, project)
		if err != nil {
...
func (r *parallelRunner) populateActualCosts(projects []*engine.Workspace) {
	if r.runCtx.Config.UsageAPIEndpoint != "" {
		logging.Logger.Debug().Msg("Retrieving actual costs from C3X Cloud")

		for _, project := range projects {
			if err := pricing.PopulateActualCosts(r.runCtx, project); err != nil {
...
func (r *parallelRunner) generateUsageFile(provider engine.Vendor) (err error) {
...
func loadRunFlags(cfg *settings.Settings, cmd *cobra.Command) error {
	...
	if cmd.Name() != "c3x" && !hasPathFlag && !hasConfigFile {
		m := fmt.Sprintf("No path specified\n\nUse the %s flag to specify the path to one of the following:\n", ui.PrimaryString("--path"))
		m += fmt.Sprintf(" - Terraform/Terragrunt directory\n - Terraform plan JSON file, see %s for how to generate this.", ui.SecondaryLinkString("https://c3x.dev/troubleshoot"))
		m += fmt.Sprintf("\n\nAlternatively, use --config-file to process multiple projects, see %s", ui.SecondaryLinkString("https://c3x.dev/config-file"))
	...
	if cmd.Flags().Changed("offline") {
		cfg.OfflineMode, _ = cmd.Flags().GetBool("offline")
	}
...
func checkRunConfig(warningWriter io.Writer, cfg *settings.Settings) error {
...
func (r *parallelRunner) buildRunEnv(projectContexts []*settings.ProjectSession, or render.Report) map[string]interface{} {
...
func newErroredProject(provider engine.Vendor, ctx *settings.ProjectSession, err error) *projectOutput {
	metadata := engine.DetectProjectMetadata(ctx.ProjectConfig.Path)
	metadata.Type = "error"
	...
	return &projectOutput{projects: []*engine.Workspace{engine.NewProject(name, metadata)}}
}
...
type runCommandMiddleware func(ctx *settings.Session, next runCommandFunc) runCommandFunc

// checkAPIKeyIsValid is a runCommandMiddleware that checks if the API key is
// valid before running the command. If a custom pricing endpoint is configured
// or the C3X_SELF_HOSTED environment variable is set, the API key is optional.
func checkAPIKeyIsValid(ctx *settings.Session, next runCommandFunc) runCommandFunc {
	return func(cmd *cobra.Command, args []string) error {
		selfHosted := ctx.Config.PricingAPIEndpoint != "" && ctx.Config.PricingAPIEndpoint != ctx.Config.DefaultPricingAPIEndpoint
		if os.Getenv("C3X_SELF_HOSTED") != "" {
			selfHosted = true
		}
		...
		if ctx.Config.APIKey == "" && !selfHosted {
			return fmt.Errorf("%s %s %s %s %s\n%s %s.\n%s %s %s",
				...
				"See",
				ui.LinkString("https://c3x.dev/support"),
				"if you continue having issues.")
		}
