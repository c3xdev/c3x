...
func authCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
...
func authLoginCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "login",
		Short: "Authenticate the CLI with your C3X account",
		Long:  "Authenticate the CLI with your C3X account",
		Example: fmt.Sprintf(`  Get a free API key, or log in to your existing account:
  ...
  Manually set the API key that your CLI should use. The API key can be retrieved from your account:
      ...
      c3x configure set api_key MY_API_KEY`, ctx.Config.DashboardEndpoint),
		ValidArgs: []string{"--", "-"},
		RunE: func(cmd *cobra.Command, _ []string) error {
			cmd.Println("We're redirecting you to our log in page, please complete that,\nand return here to continue using C3X.")
			...
			cmd.Println("\nYour account has been authenticated. Run C3X on your Terraform project by running:")
			cmd.Printf("\n  c3x breakdown --path=.\n\n")
