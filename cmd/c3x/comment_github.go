...
func commentGitHubCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "github",
		Short: "Post a C3X comment to GitHub",
		Long:  "Post a C3X comment to GitHub",
		Example: `  Update comment on a pull request:

      c3x comment github --repo my-org/my-repo --pull-request 3 --path c3x.json --github-token $GITHUB_TOKEN

  Post a new comment to a commit:

      c3x comment github --repo my-org/my-repo --commit 2ca7182 --path c3x.json --behavior hide-and-new --github-token $GITHUB_TOKEN`,
		ValidArgs: []string{"--", "-"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			commentOut, commentErr := buildCommentOutput(cmd, ctx, paths, render.MarkdownOptions{
				...
				MaxMessageSize:      render.GitHubMaxMessageSize,
			})
  ...
  delete-and-new    Delete previous matching comments and create a new comment`)
	...
	cmd.Flags().String("tag", "", "Customize hidden markdown tag used to detect comments posted by C3X")
