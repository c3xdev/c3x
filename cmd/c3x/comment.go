...
func commentBody(
	...
	baselinePath string,
	expand bool,
) (string, []domain.LabeledCaveat, error) {
	...
	if baselinePath != "" {
		...
		if err != nil {
			return "", nil, fmt.Errorf("loading baseline %s: %w", baselinePath, err)
		}
		s, err := comment.FormatCommentDiff(domain.ComputeDiff(baseline, current), expand)
		return s, current.AllCaveats(), err
	}
	...
	if planBaseline != nil {
		s, err := comment.FormatCommentDiff(domain.ComputeDiff(*planBaseline, current), expand)
		return s, current.AllCaveats(), err
	}
	s, err := comment.FormatComment(current, expand)
	return s, current.AllCaveats(), err
}
...
// lockstep.
func addCommentBehaviorFlags(cmd *cobra.Command, tag *string, recreate, expand, strict *bool) {
	...
	cmd.Flags().BoolVar(strict, "strict", false, strictHelp+" (after posting the comment, so reviewers see them)")
}

// commentOptions builds comment.Options from the shared flags, falling
// back to $C3X_COMMENT_TAG when --comment-tag is unset. The env
// fallback matches how tokens resolve and suits CI where the tag is
// templated per environment/directory.
func commentOptions(tag string, recreate bool) comment.Options {
	if tag == "" {
		tag = os.Getenv("C3X_COMMENT_TAG")
	}
	return comment.Options{Tag: tag, Recreate: recreate}
}
...
token reads from AZURE_DEVOPS_TOKEN or SYSTEM_ACCESSTOKEN.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			poster, err := comment.NewAzureDevOpsPoster(token, baseURL, target, commentOptions(commentTag, recreate))
			if err != nil {
				return err
			}
			...
			body, caveats, err := commentBody(cmd.Context(), path, resolved, varFiles, vars, baselinePath, expand)
			if err != nil {
				return err
			}
			...
			return enforceStrict(cmd, caveats, strict)
		},
	}
	...
	addCommentBehaviorFlags(cmd, &commentTag, &recreate, &expand, &strict)
	return cmd
}
...
by a dedicated implementation.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			poster, err := comment.NewBitbucketPoster(user, password, baseURL, target, commentOptions(commentTag, recreate))
			if err != nil {
				return err
			}
...
or CI_JOB_TOKEN, or pass --token.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			poster, err := comment.NewGitLabPoster(token, baseURL, target, commentOptions(commentTag, recreate))
			if err != nil {
				return err
			}
...
--token. Outside CI, supply --owner --repo --pr explicitly.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			poster, err := comment.NewGitHubPoster(token, target, commentOptions(commentTag, recreate))
			if err != nil {
				return err
			}
