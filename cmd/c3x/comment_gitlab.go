...
func commentGitLabCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "gitlab",
		Short: "Post a C3X comment to GitLab",
		Long:  "Post a C3X comment to GitLab",
		Example: `  Update comment on a merge request:

      c3x comment gitlab --repo my-org/my-repo --merge-request 3 --path c3x.json --gitlab-token $GITLAB_TOKEN
