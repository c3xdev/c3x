...
import (
	...
	"github.com/c3xdev/c3x/internal/settings/template"
...
func newGenerateConfigCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "config",
		Short: "Generate C3X config file from a template file",
		Long: `Generate C3X config file from a template file. See docs for template examples and syntax:

  https://www.c3x.dev/docs/features/config_file/#template-syntax`,
		Example: `
      c3x generate config --repo-path . --template-path c3x.yml.tmpl --out-file c3x.yml
      `,
		ValidArgs: []string{"--", "-"},
		RunE:      gen.run,
	}
	...
	cmd.Flags().StringVar(&gen.template, "template", "", "C3X template string that will generate the config-file yaml output")
	cmd.Flags().StringVar(&gen.templatePath, "template-path", "", "Path to the C3X template file that will generate the config-file yaml output")
...
func (g *generateConfigCommand) run(cmd *cobra.Command, args []string) error {
	if _, err := os.Stat(g.templatePath); g.templatePath != "" && err != nil {
		ui.PrintUsage(cmd)
		return fmt.Errorf("provided template file %q does not exist", g.templatePath)
	}
	...
	detectionOutput, err := cloud.Detect(ctx, &settings.Project{Path: wd}, false)
	if err != nil {
		if definedProjects {
	...
	var cfgFile settings.SettingsFileSpec
	...
	if err != nil {
		...
		if _, ok := err.(*settings.YamlError); ok {
...
func newGenerateCommand() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "generate",
		Short: "Generate configuration to help run C3X",
		Long:  "Generate configuration to help run C3X",
		Example: ` Generate C3X config file from a template file:

      c3x generate config --repo-path . --template-path c3x.yml.tmpl
      `,
		ValidArgs: []string{"--", "-"},
...
func unmarshalAutoDetectSection(reader io.Reader) (*settings.Settings, error) {
