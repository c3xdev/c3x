...
func TestDiffProjectName(t *testing.T) {
	...
	GoldenFileCommandTest(t, testName,
		[]string{
			"diff",
			"--config-file", path.Join("./testdata", testName, "c3x-config.yml"),
...
func TestDiffWithC3XJSON(t *testing.T) {
	dir := path.Join("./testdata", testutil.CalcGoldenFileTestdataDirName())
	GoldenFileCommandTest(
		t,
		testutil.CalcGoldenFileTestdataDirName(),
...
projects:
  ...
  - path: %s`,
	...
	configFilePath := path.Join(dir, "c3x.yml")
...
func TestDiffTerragruntSyntaxError(t *testing.T) {
	...
	GoldenFileCommandTest(
		...
		[]string{
			...
			"--config-file", filepath.Join(dir, "c3x.config.yml"),
		},
		&GoldenFileOptions{},
	)
}
