...
func configureCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "configure",
...
func configureSetCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "set <key> <value>",
		Short: "Set global configuration",
		Long:  supportedConfigureSettingsOutput("Set global configuration"),
		Example: `  Set your C3X API key:

      c3x configure set api_key MY_API_KEY

  Set your Cloud Pricing API endpoint:

      c3x	configure set pricing_api_endpoint https://cloud-pricing-api

  Set your preferred currency code (ISO 4217):

      c3x	configure set currency EUR
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "enable_dashboard":
				...
				if b && ctx.Config.IsSelfHosted() {
					return errors.New("The dashboard is part of C3X's hosted services. Contact hello@c3x.dev for help")
				}
			...
			case "enable_cloud":
				...
				if b && ctx.Config.IsSelfHosted() {
					return errors.New("C3X Cloud is part of C3X's hosted services. Contact hello@c3x.dev for help")
				}
...
func configureGetCmd(ctx *settings.Session) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "get <key>",
		Short: "Get global configuration",
		Long:  supportedConfigureSettingsOutput("Get global configuration"),
		Example: `  Get your saved C3X API key:

      c3x configure get api_key

  Get your saved Cloud Pricing API endpoint:

      c3x	configure get pricing_api_endpoint

  Get your preferred currency:

      c3x	configure get currency
`,
		Args: func(cmd *cobra.Command, args []string) error {
			if len(args) > 1 {
				return errors.New("Too many arguments")
			...
			case "pricing_api_endpoint":
				...
				if value == "" {
					msg := fmt.Sprintf("No Cloud Pricing API endpoint in your saved config (%s).\nSet an API key using %s.",
						...
						ui.PrimaryString("c3x configure set pricing_api_endpoint https://cloud-pricing-api"),
					)
			...
			case "api_key":
				...
				if value == "" {
					msg := fmt.Sprintf("No API key in your saved config (%s).\nSet an API key using %s.",
						settings.CredentialsFilePath(),
						ui.PrimaryString("c3x configure set api_key MY_API_KEY"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "currency":
				...
				if value == "" {
					msg := fmt.Sprintf("No currency in your saved config (%s), defaulting to USD.\nSet a currency using %s.",
						...
						ui.PrimaryString("c3x configure set currency CURRENCY"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "tls_insecure_skip_verify":
				...
				if value == "" {
					msg := fmt.Sprintf("Skipping TLS verification is not set in your saved config (%s).\nSet it using %s.",
						settings.ConfigurationFilePath(),
						ui.PrimaryString("c3x configure set tls_insecure_skip_verify true"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "tls_ca_cert_file":
				...
				if value == "" {
					msg := fmt.Sprintf("No CA cert file in your saved config (%s).\nSet a CA certificate using %s.",
						settings.ConfigurationFilePath(),
						ui.PrimaryString("c3x configure set tls_ca_cert_file /path/to/ca.crt"),
					)
					logging.Logger.Warn().Msg(msg)
				}
			case "enable_dashboard":
