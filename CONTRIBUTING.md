...
[Join our community Slack channel](https://www.c3x.dev/community-chat), we are a friendly bunch and happy to help you get started :)
...
> [!NOTE]
>
> C3X is transitioning to a GitHub Discussions-based workflow for discussions
> about bugs, feature requests and other topics. Issues are for actionable items that
> are ready to be worked on.

The overall process for contributing to C3X is:

1. Check the [project board](https://github.com/c3xdev/c3x/projects/2) to see if there is something you'd like to work on; these are the issues we'd like to focus on in the near future. The issue labels should help you to find an issue to work on. There are also [other issues](https://github.com/c3xdev/c3x/issues) and [discussions](https://github.com/c3xdev/c3x/discussions) that you might like to check.
2. If there's no issue for what you want to work on, create a [new discussion](https://github.com/c3xdev/c3x/discussions) thread about the topic. Please put as much as details as you think is necessary, the use-case context is especially helpful if you'd like to receive good feedback. When we have enough detail and are ready to prioritize the work we will then create an issue for it.
...
5. If it's your first PR to the C3X org, a bot will leave a comment asking you to follow a quick step to sign our Contributor License Agreement.
...
```sh
make run ARGS="breakdown --path examples/terraform --usage-file=examples/terraform/c3x-usage.yml"
```

This will use your existing C3X API key; register for a [free API key](https://www.c3x.dev/docs/#2-get-api-key) key if you don't have one already.
...
```sh
C3X_LOG_LEVEL=warn go test -v -cover ./internal/cloud/terraform/aws/ebs_volume_test.go
...
1. Go to `https://github.com/<YOUR GITHUB NAME>/c3x/settings/secrets/actions`.
...
2. Go to https://pricing.api.c3x.dev/graphql
3. Set your `X-API-Key` using the browser extension
4. Run GraphQL queries to find the correct products. Examples can be found here: https://www.c3x.dev/docs/supported_resources/cloud_pricing_api/
...
Consult [PR #2628](https://github.com/c3xdev/c3x/pull/2628) as an example.

1. In [internal/catalog/aws/util.go](internal/catalog/aws/util.go), add the new region's information to `RegionMapping`, `RegionCodeMapping`, and `RegionsUsage` as needed.
2. In [internal/catalog/aws/global_accelerator_endpoint_group.go](internal/catalog/aws/global_accelerator_endpoint_group.go), update `regionCodeMapping` as needed.
3. In [internal/catalog/aws/cloudfront_distribution.go](internal/catalog/aws/cloudfront_distribution.go), update `regionShieldMapping` as needed.
4. In [internal/cloud/terraform/aws/testdata/data_transfer_test/data_transfer_test.usage.yml](internal/cloud/terraform/aws/testdata/data_transfer_test/data_transfer_test.usage.yml), add a usage block for data transfer.
5. Update [internal/cloud/terraform/aws/testdata/data_transfer_test/data_transfer_test.golden](internal/cloud/terraform/aws/testdata/data_transfer_test/data_transfer_test.golden) by running `ARGS="-run TestDataTransferGoldenFile -v -update" make test_aws`.
6. Update [internal/iac/hcl/zones_aws.go]:
...
- Is the [c3x-usage-example.yml](https://github.com/c3xdev/c3x/blob/master/c3x-usage-example.yml) file updated with any new usage file parameters and descriptions?
...
- Any [docs](https://www.c3x.dev/docs/) pages need to be updated? e.g. the [supported resources](https://github.com/c3xdev/docs/blob/master/docs/supported_resources/) pages. If so, please open a PR so it can be merged after the CLI is released.
