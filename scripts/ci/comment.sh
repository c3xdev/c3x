...
#
# Please follow our guide: https://www.c3x.dev/docs/guides/gitlab_ci_migration/ to migrate
# to our new GitHub Actions integration: https://gitlab.com/c3x/c3x-gitlab-ci/
#
...
echo "Please visit https://www.c3x.dev/docs/guides/gitlab_ci_migration/ for instructions on how to upgrade."
echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
# Build flags for C3X to handle multiple paths
if [ "$#" -eq 0 ]; then
  echo "Expecting at least one C3X JSON file as argument"
  exit 1
fi
...
# Run c3x output to generate the comment
export C3X_CI_POST_CONDITION=$behavior
body_file=$(mktemp)
# shellcheck disable=SC2086
c3x_cmd="c3x output $input_path_flags --format $format --show-skipped --out-file $body_file"
echo "Running c3x output:" >&2
echo "  $c3x_cmd" >&2
eval "$c3x_cmd"
