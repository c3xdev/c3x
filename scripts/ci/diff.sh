...
#
# Please visit : https://www.c3x.dev/docs/integrations/cicd/ to migrate to
...
echo "Please visit https://github.com/c3xdev/c3x/blob/master/scripts/ci/README.md for instructions on how to upgrade."
echo "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!"
...
process_args () {
  ...
  export C3X_CI_DIFF=true
  export C3X_CI_POST_CONDITION=$post_condition
  export C3X_CI_PERCENTAGE_THRESHOLD=$percentage_threshold
...
build_msg () {
  ...
  if [ "$include_html" = true ]; then
    ...
    for (( i = 0; i < project_count; i++ )); do
      diff_resources=$(jq '.projects['"$i"'].diff.resources[]' c3x_breakdown.json)
      if  [ -n "$diff_resources" ] || [ $project_count -eq 1 ]; then
        msg+="$(build_project_row "$i")"
      else
        if [ -z "$skipped_projects" ]; then
          skipped_projects="$(jq -r '.projects['"$i"'].name' c3x_breakdown.json)"
        else
          skipped_projects="$skipped_projects, $(jq -r '.projects['"$i"'].name' c3x_breakdown.json)"
        fi
      fi
    done
    ...
    msg+="<details>\n"
    msg+="  <summary><strong>C3X output</strong></summary>\n"
  else
  ...
  if [ "$include_html" = true ]; then
    ...
    msg+="<sub>\n"
    msg+="  Is this comment useful? <a href=\"https://dashboard.c3x.dev/feedback/redirect?value=yes\" rel=\"noopener noreferrer\" target=\"_blank\">Yes</a>, <a href=\"https://dashboard.c3x.dev/feedback/redirect?value=no\" rel=\"noopener noreferrer\" target=\"_blank\">No</a>, <a href=\"https://dashboard.c3x.dev/feedback/redirect?value=other\" rel=\"noopener noreferrer\" target=\"_blank\">Other</a>\n"
    msg+="</sub>\n"
  fi
...
build_project_row () {
  ...
  max_name_length=64
  name=$(jq -r '.projects['"$i"'].name' c3x_breakdown.json)
  ...
  past_monthly_cost=$(jq -r '.projects['"$i"'].pastBreakdown.totalMonthlyCost' c3x_breakdown.json)
  monthly_cost=$(jq -r '.projects['"$i"'].breakdown.totalMonthlyCost' c3x_breakdown.json)
  diff_monthly_cost=$(jq -r '.projects['"$i"'].diff.totalMonthlyCost' c3x_breakdown.json)
...
post_to_azure_devops () {
  if [ "$BUILD_REASON" = "PullRequest" ]; then
    if [ "$BUILD_REPOSITORY_PROVIDER" = "GitHub" ]; then
      ...
      echo "Posting comments to Azure DevOps $BUILD_REPOSITORY_PROVIDER is not supported, email hello@c3x.dev for help"
    fi
  else
    echo "Posting comment to Azure DevOps $BUILD_REASON is not supported, email hello@c3x.dev for help"
  fi
}
