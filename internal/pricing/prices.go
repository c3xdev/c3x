...
// RateLookup provides a thread-safe way to aggregate 'price not found'
...
func (p *RateLookup) addNotFoundResult(result apiclient.PriceQueryResult) {
	p.mux.Lock()
	defer p.mux.Unlock()
...
func (p *RateLookup) getPricesConcurrent(resources []*engine.Estimate) error {
...
func (p *RateLookup) getPrices(req apiclient.BatchRequest) error {
...
func (p *RateLookup) logPriceLookupErr(cc *engine.LineItem, format string, v ...interface{}) {
...
func (p *RateLookup) setCostComponentPrice(result apiclient.PriceQueryResult) {
