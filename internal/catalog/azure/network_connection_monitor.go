...
func (r *NetworkConnectionMonitor) UsageSchema() []*engine.ConsumptionField {
	return []*engine.ConsumptionField{
		{Key: "tests", DefaultValue: 0, ValueType: engine.Int64},
	}
}

// PopulateUsage parses the u engine.ConsumptionProfile into the NetworkConnectionMonitor.
// It uses the `c3x_usage` struct tags to populate data into the NetworkConnectionMonitor.
func (r *NetworkConnectionMonitor) PopulateUsage(u *engine.ConsumptionProfile) {
	catalog.PopulateArgsWithUsage(r, u)
}

// BuildResource builds a engine.Estimate from a valid NetworkConnectionMonitor struct.
...
func (r *NetworkConnectionMonitor) testsCostComponents() []*engine.LineItem {
...
func (r *NetworkConnectionMonitor) buildTestsCostComponent(suffix string, startUsage string, qty *decimal.Decimal) *engine.LineItem {
