...
type frontdoorOutboundDataTransferUsage struct {
	USGovZone1MonthlyTransferGB *float64 `c3x_usage:"us_gov"`
	Zone1MonthlyTransferGB      *float64 `c3x_usage:"north_america_europe_africa"`
	Zone2MonthlyTransferGB      *float64 `c3x_usage:"asia_pacific"`
	Zone3MonthlyTransferGB      *float64 `c3x_usage:"south_america"`
	Zone4MonthlyTransferGB      *float64 `c3x_usage:"australia"`
	Zone5MonthlyTransferGB      *float64 `c3x_usage:"india"`
}
...
func (r *Frontdoor) UsageSchema() []*engine.ConsumptionField {
	return []*engine.ConsumptionField{
		{Key: "monthly_inbound_data_transfer_gb", DefaultValue: 0, ValueType: engine.Float64},
		{
...
var frontdoorOutboundDataUsageSchema = []*engine.ConsumptionField{
	{Key: "us_gov", DefaultValue: 0, ValueType: engine.Float64},
	{Key: "north_america_europe_africa", DefaultValue: 0, ValueType: engine.Float64},
...
// PopulateUsage parses the u engine.ConsumptionProfile into the Frontdoor.
// It uses the `c3x_usage` struct tags to populate data into the Frontdoor.
func (r *Frontdoor) PopulateUsage(u *engine.ConsumptionProfile) {
	catalog.PopulateArgsWithUsage(r, u)
}

// BuildResource builds a engine.Estimate from valid Frontdoor data.
// This method is called after the resource is initialised by an IaC provider.
func (r *Frontdoor) BuildResource() *engine.Estimate {
	...
	outboundTransferSubResource := &engine.Estimate{
	...
	return &engine.Estimate{
		...
		SubResources:   []*engine.Estimate{outboundTransferSubResource},
	}
}

// routingRulesCostComponents returns cost components for defined routing rules.
// The pricing depends on rules quantity.
func (r *Frontdoor) routingRulesCostComponents() []*engine.LineItem {
	costComponents := []*engine.LineItem{}
...
func (r *Frontdoor) frontendHostsCostComponents() []*engine.LineItem {
	costComponents := []*engine.LineItem{}
...
func (r *Frontdoor) inboundDataTransferCostComponents() []*engine.LineItem {
...
func (r *Frontdoor) outboundDataTransferCostComponents() []*engine.LineItem {
	if r.isGovZone(r.Region) {
...
func (r *Frontdoor) govOutboundDataTransferCostComponents() []*engine.LineItem {
	costComponents := []*engine.LineItem{}
...
func (r *Frontdoor) buildOutboundDataTransferCostComponent(name, startUsage string, quantity *decimal.Decimal) *engine.LineItem {
	return &engine.LineItem{
		Name:            name,
		Unit:            "GB",
		UnitMultiplier:  decimal.NewFromInt(1),
...
// Standard/Premium).
func (r *Frontdoor) buildProductFilter(meterName string) *engine.ProductSelector {
