...
func (r *AppConfiguration) UsageSchema() []*engine.ConsumptionField {
	return []*engine.ConsumptionField{
		{Key: "monthly_additional_requests", ValueType: engine.Int64, DefaultValue: 0},
	}
}

// PopulateUsage parses the u engine.ConsumptionProfile into the AppConfiguration.
// It uses the `c3x_usage` struct tags to populate data into the AppConfiguration.
func (r *AppConfiguration) PopulateUsage(u *engine.ConsumptionProfile) {
	catalog.PopulateArgsWithUsage(r, u)
}

// BuildResource builds a engine.Estimate from a valid AppConfiguration struct.
...
func (r *AppConfiguration) requestCostComponent(titledSku string) *engine.LineItem {
	...
	requestComponent := &engine.LineItem{
...
func (r *AppConfiguration) instanceCostComponent(sku string) *engine.LineItem {
