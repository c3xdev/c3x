...
func (d *DirectoryServiceDirectory) UsageSchema() []*engine.ConsumptionField {
	return []*engine.ConsumptionField{
		{Key: "additional_domain_controllers", DefaultValue: 0, ValueType: engine.Float64},
		{Key: "shared_accounts", DefaultValue: 0, ValueType: engine.Float64},
	}
}

// PopulateUsage parses the u engine.Usage into the DirectoryServiceDirectory.
// It uses the `c3x_usage` struct tags to populate data into the DirectoryServiceDirectory.
func (d *DirectoryServiceDirectory) PopulateUsage(u *engine.ConsumptionProfile) {
	catalog.PopulateArgsWithUsage(d, u)
}
...
func (d DirectoryServiceDirectory) domainControllerCostComponent(amount float64, name, size string) *engine.LineItem {
...
func (d DirectoryServiceDirectory) additionalDomainControllerCostComponent(amount float64, size string) *engine.LineItem {
	return &engine.LineItem{
...
func (d DirectoryServiceDirectory) getAttributeFiltersForDirectory(size string) []*engine.AttributeMatch {
	if d.Type == dtMicrosoftAD {
		return []*engine.AttributeMatch{
			{Key: "directorySize", Value: strPtr(size)},
