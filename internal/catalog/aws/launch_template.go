...
func (r *LaunchTemplate) BuildResource() *engine.Estimate {
	if strings.ToLower(r.Tenancy) == "host" {
		logging.Logger.Warn().Msgf("Skipping resource %s. C3X currently does not support host tenancy for AWS Launch Templates", r.Address)
		return nil
	...
	engine.ScaleQuantities(res, decimal.NewFromInt(instanceCount))
	...
	if spotCount > 0 {
		...
		res.CostComponents = append([]*engine.LineItem{c}, res.CostComponents...)
	}
