...
func (a *EKSNodeGroup) getUsageSchemaWithDefaultInstanceCount() []*engine.ConsumptionField {
	if a.InstanceCount == nil || *a.InstanceCount == 0 {
		return EKSNodeGroupUsageSchema
	}

	usageSchema := make([]*engine.ConsumptionField, 0, len(EKSNodeGroupUsageSchema))
	for _, u := range EKSNodeGroupUsageSchema {
		if u.Key == "instances" {
			usageSchema = append(usageSchema, &engine.ConsumptionField{Key: "instances", DefaultValue: intVal(a.InstanceCount), ValueType: engine.Int64})
		} else {
...
func (a *EKSNodeGroup) BuildResource() *engine.Estimate {
	...
	if a.LaunchTemplate != nil {
		...
		engine.ScaleQuantities(r, decimal.NewFromInt(qty))
	}
