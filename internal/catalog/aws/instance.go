...
func (a *Instance) BuildResource() *engine.Estimate {
	if strings.ToLower(a.Tenancy) == "host" {
		if a.HasHost {
			...
			logging.Logger.Warn().Msgf("Skipping resource %s. C3X currently does not support host tenancy for AWS EC2 instances without Host ID set up", a.Address)
			return nil
		}
...
func (a *Instance) computeCostComponent() *engine.LineItem {
...
func (a *Instance) ebsOptimizedCostComponent() *engine.LineItem {
	/**
...
func (a *Instance) detailedMonitoringCostComponent() *engine.LineItem {
	return &engine.LineItem{
...
func (a *Instance) elasticInferenceAcceleratorCostComponent() *engine.LineItem {
	/**
...
func (a *Instance) cpuCreditCostComponent(instanceFamily string) *engine.LineItem {
	qty := decimal.Zero
...
func (r ec2ReservationResolver) PriceFilter() (*engine.RateSelector, error) {
