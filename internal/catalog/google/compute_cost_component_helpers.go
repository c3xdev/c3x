...
func computeCostComponents(region, machineType string, purchaseOption string, instanceCount int64, monthlyHours *float64) ([]*engine.LineItem, error) {
	if strings.HasPrefix(strings.ToLower(machineType), "e2-custom") {
		return nil, errors.New("C3X currently does not support E2 custom instances")
	}
...
func getSustainedUseDiscount(hours float64, rates sudRates) float64 {
	...
	totalHoursInMonth, _ := engine.HourToMonthUnitMultiplier.Float64()
...
// Compute catalog.
func bootDiskCostComponent(region string, diskSize float64, diskType string) *engine.LineItem {
...
// Compute catalog.
func scratchDiskCostComponent(region string, purchaseOption string, count int) *engine.LineItem {
...
// Compute catalog.
func computeDiskCostComponent(region string, diskType string, diskSize float64, instanceCount int64) *engine.LineItem {
...
func computeDiskIOPSCostComponent(region string, diskType string, diskSize float64, instanceCount int64, iops int64) *engine.LineItem {
...
// guestAcceleratorCostComponent returns a cost component for Guest Accelerator usage for Compute catalog.
// Callers should be aware guestAcceleratorCostComponent returns nil if the provided guestAcceleratorType is not supported.
func guestAcceleratorCostComponent(region string, purchaseOption string, guestAcceleratorType string, guestAcceleratorCount int64, instanceCount int64, monthlyHours *float64) *engine.LineItem {
...
func storageImageCostComponent(region string, description string, storageSize *decimal.Decimal) *engine.LineItem {
	return &engine.LineItem{
		Name:            "Storage",
		Unit:            "GB",
		UnitMultiplier:  decimal.NewFromInt(1),
