...
// PopulateUsage parses the u engine.ConsumptionProfile into the ArtifactRegistryRepository.
// It uses the `c3x_usage` struct tags to populate data into the ArtifactRegistryRepository.
func (r *ArtifactRegistryRepository) PopulateUsage(u *engine.ConsumptionProfile) {
	catalog.PopulateArgsWithUsage(r, u)
}

// BuildResource builds a engine.Estimate from a valid ArtifactRegistryRepository struct. It returns ArtifactRegistryRepository
// as a engine.Estimate with two main cost components: storage costs & egress costs.
//
// Storage costs:
//
...
func (r *ArtifactRegistryRepository) internalEgressComponents() []*engine.LineItem {
	filters := r.toEgressFilters()
	components := make([]*engine.LineItem, 0, len(filters))
	for _, v := range filters {
		components = append(components, &engine.LineItem{
			Name:            v.name,
			Unit:            "GB",
...
func (r *ArtifactRegistryRepository) storageCostComponent() *engine.LineItem {
...
// a set of cost components. One artifactRegistryEgressFilters is used by one engine.LineItem.
//
