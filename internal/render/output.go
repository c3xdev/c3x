...
// ToSchemaProject generates a engine.Workspace from a Project. The created engine.Workspace is not suitable to be
// used outside simple engine.Workspace to engine.Workspace comparisons. It contains missing information
// that cannot be inferred from a Project.
func (p *Project) ToSchemaProject() *engine.Workspace {
	var pastResources []*engine.Estimate
	...
	var clonedMetadata *engine.WorkspaceMeta
	if p.Metadata != nil {
		m := *p.Metadata
		clonedMetadata = &m
	}
...
func convertOutputResources(outResources []Resource, skip bool) []*engine.Estimate {
	resources := make([]*engine.Estimate, len(outResources))

	for i, resource := range outResources {

		var tagProp *engine.TagPropagation
		if resource.TagPropagation != nil {
			tagProp = &engine.TagPropagation{
...
func convertCostComponents(outComponents []CostComponent) []*engine.LineItem {
	components := make([]*engine.LineItem, len(outComponents))
...
func convertActualCosts(outActualCosts []ActualCosts) []*engine.ObservedCost {
	actualCosts := make([]*engine.ObservedCost, len(outActualCosts))

	for i, ac := range outActualCosts {
		sac := &engine.ObservedCost{
...
var exampleProjectsRegex = regexp.MustCompile(`^c3x\/(c3x\/examples|example-terraform)\/`)
...
func (r *Report) HasDiff() bool {
	for _, p := range r.Projects {
		if p.Diff != nil && ((p.Diff.TotalMonthlyCost != nil && !p.Diff.TotalMonthlyCost.IsZero()) || len(p.Diff.Resources) != 0) {
			return true
		}
	}
...
// a uniform "Policies" section of the c3x comment.
...
func outputBreakdown(c *settings.Settings, resources []*engine.Estimate) *Breakdown {
...
func outputResource(r *engine.Estimate) Resource {
...
func newResource(r *engine.Estimate, comps []CostComponent, actualCosts []ActualCosts, subresources []Resource) Resource {
...
func outputCostComponents(costComponents []*engine.LineItem) []CostComponent {
...
func outputActualCosts(actualCosts []*engine.ObservedCost) []ActualCosts {
...
func (r *Report) summaryMessage(showSkipped bool) string {
	msg := ""
	...
	if r.Summary.TotalUnsupportedResources != nil && *r.Summary.TotalUnsupportedResources > 0 {
		...
		if showSkipped {
			msg += fmt.Sprintf(", see %s:", ui.SecondaryLinkString("https://c3x.dev/requested-resources"))
...
func sortResources(resources []Resource, groupKey string) {
	sort.Slice(resources, func(i, j int) bool {
		...
		if groupKey != "" && resources[i].Metadata[groupKey] != resources[j].Metadata[groupKey] {
			iVal, iOk := resources[i].Metadata[groupKey].(string)
			jVal, jOk := resources[j].Metadata[groupKey].(string)
			if iOk && jOk {
				return iVal < jVal
			}
		}
...
func usageCostsEnabled(out Report) bool {
