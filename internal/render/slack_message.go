...
func slackAllProjectsSummaryBlock(out Report, currency string) []*slack.TextBlockObject {
...
func ToSlackMessage(out Report, opts Options) ([]byte, error) {
	...
	blocks := []slack.Block{
		slack.NewSectionBlock(
			&slack.TextBlockObject{
				...
				Text: fmt.Sprintf("💰 C3X estimate: *%s*", formatCostChangeSentence(out.Currency, out.PastTotalMonthlyCost, out.TotalMonthlyCost, true)),
			},
	...
	diffMsg := fmt.Sprintf("*C3X output*\n```%s```", ui.StripColor(string(diff)))
