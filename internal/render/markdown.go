...
var (
	usageFileDocsURL             = "https://www.c3x.dev/docs/features/usage_based_resources/#c3x-usageyml"
	usageDefaultsDocsURL         = "https://www.c3x.dev/docs/features/usage_based_resources"
	usageDefaultsDashboardSuffix = "settings/usage-defaults"
	cloudURLRegex                = regexp.MustCompile(`(https?://[^/]+/org/[^/]+/)`)
)

func usageCostsMessage(out Report, useLinks bool) string {
...
func cloudSettingsStr(out Report, useMarkdownLinks bool) string {
	if !useMarkdownLinks {
		return "C3X Cloud settings"
	}
	...
	return fmt.Sprintf("[C3X Cloud settings](%s)", usageDefaultsURL)
}
...
func handleUsageApiEnabledMessage(out Report, useMarkdownLinks bool) string {
	if out.Metadata.UsageFilePath != "" || out.Metadata.ConfigFileHasUsageFile {
		return fmt.Sprintf("*Usage costs were estimated by merging c3x-usage.yml and %s.", cloudSettingsStr(out, useMarkdownLinks))
	}
...
func handleUsageApiDisabledMessage(out Report, useMarkdownLinks bool) string {
	if out.Metadata.UsageFilePath != "" || out.Metadata.ConfigFileHasUsageFile {
		return fmt.Sprintf("*Usage costs were estimated using c3x-usage.yml, see %s for other options.", usageDocsStr(useMarkdownLinks))
	}
...
func costsDetailsMessage(out Report) string {
	var msgs []string
