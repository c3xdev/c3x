...
// validates that the C3X JSON body is valid by checking the that the version attribute is within a supported range.
func Load(p string) (Report, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		return out, errors.New("C3X JSON file does not exist, generate it by running the following command then try again:\nc3x breakdown --path /code --format json --out-file c3x-base.json")
	}

	data, err := os.ReadFile(p)
	if err != nil {
		return out, fmt.Errorf("error reading C3X JSON file %w", err)
	}

	err = json.Unmarshal(data, &out)
	if err != nil {
		return out, fmt.Errorf("invalid C3X JSON file %w, generate it by running the following command then try again:\nc3x breakdown --path /code --format json --out-file c3x-base.json", err)
	}

	if !checkOutputVersion(out.Version) {
		return out, fmt.Errorf("invalid C3X JSON file version. Supported versions are %s ≤ x ≤ %s", minOutputVersion, maxOutputVersion)
	}

	for i, p := range out.Projects {
		if p.Metadata == nil {
			p.Metadata = &engine.WorkspaceMeta{}
			out.Projects[i] = p
		}
...
func CompareTo(c *settings.Settings, current, prior Report) (Report, error) {
	...
	priorProjects := make(map[string]*engine.Workspace)
	for _, p := range prior.Projects {
		if _, ok := priorProjects[p.LabelWithMetadata()]; ok {
			return Report{}, fmt.Errorf("Invalid --compare-to C3X JSON, found duplicate project name %s", p.LabelWithMetadata())
		}

		priorProjects[p.LabelWithMetadata()] = p.ToSchemaProject()
	}

	var schemaProjects engine.Projects
	for _, p := range current.Projects {
		...
		if v, ok := priorProjects[metadata]; ok {
			if !p.Metadata.HasErrors() && !v.Metadata.HasErrors() {
				...
				scp.Diff = engine.ComputeDiff(scp.PastResources, scp.Resources)
			}
			...
			for _, pastE := range v.Metadata.Errors {
				if engine.IsEmptyPathTypeError(pastE) {
...
func findChildrenOfErroredProject(p Project, projects map[string]*engine.Workspace) []string {
...
func Combine(inputs []ReportInput) (Report, error) {
	...
	if invalidMetadata {
		return combined, clierror.NewWarningF(
			"combining C3X JSON for different VCS repositories %s. Using %s as the top-level repository in the outputted JSON",
...
func checkCurrency(inputCurrency, fileCurrency string) (string, error) {
	...
	if inputCurrency != fileCurrency {
		return "", fmt.Errorf("Invalid C3X JSON file currency mismatch.  Can't combine %s and %s", inputCurrency, fileCurrency)
	}
