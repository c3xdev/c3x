...
func savingsFromCosts(current, suggested *decimal.Decimal) (*decimal.Decimal, *decimal.Decimal) {
	if current == nil || suggested == nil {
		return nil, nil
	}
	savings := current.Sub(*suggested)
	if current.IsPositive() {
		pct := savings.Div(*current).Mul(decimal.NewFromInt(100))
		return &savings, &pct
	}
	return &savings, nil
}
