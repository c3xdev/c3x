...
resource "aws_instance" "with_usage" {
  ami           = "ami-0c55b159cbfafe1f0"
  instance_type = "m5.4xlarge" # <<<<< Try changing this to m5.8xlarge to compare the costs
...
resource "aws_s3_bucket" "with_usage" {
  bucket = "with_usage"
}
...
resource "aws_cloudfront_distribution" "with_usage" {
  origin {
    domain_name = aws_s3_bucket.with_usage.bucket_regional_domain_name
    origin_id   = local.s3_origin_id
