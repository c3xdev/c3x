...
func (r *ResourceUsage) MergeResourceUsage(src *ResourceUsage) {
	...
	destItemMap := make(map[string]*engine.ConsumptionField, len(r.Items))
	for _, item := range r.Items {
	...
	for _, srcItem := range src.Items {
		...
		if !ok {
			destItem = &engine.ConsumptionField{Key: srcItem.Key}
...
func mapUsageItem(item *engine.ConsumptionField) interface{} {
	if item.ValueType == engine.SubResourceUsage {
...
func ResourceUsagesFromYAML(raw yamlv3.Node) ([]*ResourceUsage, error) {
	...
	for i := 0; i < len(raw.Content); i += 2 {
		...
		resourceUsage := &ResourceUsage{
			...
			Items: make([]*engine.ConsumptionField, 0, len(resourceValNode.Content)/2),
		}
...
// usageItemFromYAML item turns a YAML key node and a YAML value node into a *engine.ConsumptionField. This function supports recursion
// to allow for YAML map nodes to be parsed into nested sets of engine.ConsumptionField
//
// e.g. given:
//
//	keyNode: &yaml.Node{
...
//			},
//		}
func usageItemFromYAML(keyNode *yamlv3.Node, valNode *yamlv3.Node) (*engine.ConsumptionField, error) {
	...
	var value interface{}
	var usageValueType engine.UsageVariableType

	if valNode.ShortTag() == "!!map" {
		usageValueType = engine.SubResourceUsage
		...
		items := make([]*engine.ConsumptionField, 0, len(valNode.Content)/2)
		...
		case "!!int":
			usageValueType = engine.Int64

		case "!!float":
			usageValueType = engine.Float64

		default:
			usageValueType = engine.String
		}
	}
