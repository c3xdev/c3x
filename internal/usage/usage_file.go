...
func (u *UsageFile) WriteToPath(path string) error {
	...
	root := &yamlv3.Node{
		Kind: yamlv3.MappingNode,
		HeadComment: `You can use this file to define resource usage estimates for C3X to use when calculating
the cost of usage-based resource, such as AWS S3 or Lambda.
` + "`c3x breakdown --usage-file c3x-usage.yml [other flags]`" + `
See https://c3x.dev/usage-file/ for docs`,
	}
...
func (u *UsageFile) ToUsageDataMap() engine.ConsumptionMap {
	...
	return engine.NewUsageMapFromInterface(m)
}
...
func findInvalidKeys(item *engine.ConsumptionField, refMap map[string]interface{}) []string {
	invalidKeys := make([]string, 0)

	if refVal, ok := refMap[item.Key]; !ok {
		invalidKeys = append(invalidKeys, item.Key)
	} else if item.ValueType == engine.SubResourceUsage && item.Value != nil {
