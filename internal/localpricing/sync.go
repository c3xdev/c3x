...
func Sync(opts SyncOptions) error {
	...
	opts.Store.SetMetadata("last_sync", time.Now().UTC().Format(time.RFC3339))
	return nil
}
...
func downloadAndStoreAWSService(store *Store, service, url string) error {
	...
	for sku, product := range data.Products {
		...
		store.UpsertProduct("aws", region, service, product.ProductFamily, sku, string(attrsJSON), string(pricesJSON))
	}
...
func syncAzure(opts SyncOptions) error {
	...
	for nextPage != "" {
		...
		for _, item := range result.Items {
			...
			opts.Store.UpsertProduct("azure", item.ArmRegionName, item.ServiceName, item.ProductName, sku, string(attrsJSON), string(pricesJSON))
		}
		...
		for _, sku := range result.SKUs {
			...
			if len(sku.PricingInfo) > 0 {
				...
				for _, tier := range expr.TieredRates {
					usd := fmt.Sprintf("%s.%09d", tier.UnitPrice.Units, tier.UnitPrice.Nanos)
					// Normalize: parse and re-format to remove leading zeros in nanos
					var unitsVal int64
					fmt.Sscanf(tier.UnitPrice.Units, "%d", &unitsVal)
			...
			for _, region := range sku.ServiceRegions {
				...
				opts.Store.UpsertProduct("gcp", region, svcName, sku.Category.ResourceFamily, regionSKU, string(attrsJSON), string(pricesJSON))
			}
