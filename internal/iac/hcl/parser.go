...
import (
	...
	"github.com/c3xdev/c3x/internal/extclient"
...
func OptionWithSpaceliftRemoteVarLoader(ctx *settings.ProjectSession) Option {
	return func(p *Parser) {
...
// in the C3X config file.
func (p *Parser) ProjectName() string {
	if p.projectName != "" {
		return p.projectName
	}

	name := settings.CleanProjectName(p.RelativePath())
