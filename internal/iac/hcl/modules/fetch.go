...
func NewPackageFetcher(remoteCache RemoteCache, logger zerolog.Logger, opts ...PackageFetcherOpts) *PackageFetcher {
	...
	if proxy := os.Getenv("C3X_REGISTRY_PROXY"); proxy != "" {
...
// fetch downloads the remote module using the modsource library
// See: https://github.com/hashicorp/modsource
...
func (p *PackageFetcher) fetchFromRemote(moduleAddr, dest string) (bool, error) {
	...
	// But is not in the list of default compressors here: https://github.com/hashicorp/modsource/blob/main/decompress.go#L32
...
// expected to be an url that has been transformed by a modsource Detect pass,
