...
// .c3x/terraform_modules directory. We could implement a global cache in the future, but for now have decided
// to go with the same approach as Terraform.
type ModuleLoader struct {
	// cachePath is the path to the directory that C3X will download modules to.
	// This is normally the top level directory of a multi-project environment, where the
	// C3X config file resides or project auto-detection starts from.
	cachePath      string
	cache          *Cache
...
func (m *ModuleLoader) manifestFilePath(projectPath string) string {
	...
	return filepath.Join(m.cachePath, ".c3x/terraform_modules/", fmt.Sprintf("manifest-%x.json", sum))
}
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		if err == nil {
			...
			if err == nil {
				...
				// we'll look up the modules based on the cache path at the C3X root (where the c3x.yml
...
func RecursivelyAddDirsToSparseCheckout(repoRoot string, sourceURL string, packageFetcher *PackageFetcher, existingDirs []string, dirs []string, mu *sync.Mutex, logger zerolog.Logger, depth int) error {
	...
	tmpDir, err := os.MkdirTemp(os.TempDir(), "c3x-sparse-checkout")
	if err != nil {
...
func (m *ModuleLoader) loadRegistryModule(key string, source string, version string) (*ManifestModule, error) {
	...
	if err != nil {
		return nil, engine.NewPrivateRegistryDiag(source, nil, err)
	}
	...
	if lookupResult.OK {
		...
		if err != nil {
			return nil, engine.NewPrivateRegistryDiag(source, strPtr(lookupResult.ModuleURL.Location), err)
		}
...
func (m *ModuleLoader) loadRemoteModule(key string, source string) (*ManifestModule, error) {
	...
	if os.Getenv("C3X_SPARSE_CHECKOUT") == "true" {
	...
	if err != nil {
		_ = os.RemoveAll(dest)
		return nil, engine.NewFailedDownloadDiagnostic(source, err)
	}
...
func (m *ModuleLoader) MapSourceWithRegex(source string) (SourceMapResult, error) {
	if len(m.sourceMapRegex) > 0 {
		mappedSource, err := settings.ApplyRegexMapping(m.sourceMapRegex, source)
		if err != nil {
			return SourceMapResult{}, err
		}
