...
import (
	...
	"github.com/c3xdev/c3x/internal/iac/hcl/funcs"
...
// Most importantly a Block directly corresponds to a engine.Estimate.
//
...
type Block struct {
	...
	HCLBlock *hcl.Block
	// UniqueAttrs specifies c3x specific unique attributes that will be appended to the block values.
...
func (b BlockBuilder) NewBlock(filename string, rootPath string, hclBlock *hcl.Block, ctx *Context, parent *Block, moduleBlock *Block) *Block {
	...
	isLoggingVerbose := strings.TrimSpace(os.Getenv("C3X_HCL_DEBUG_VERBOSE")) == "true"
...
func newArnAttribute(addressSha, name string, withCount bool, withEach bool) *hcl.Attribute {
	...
	// C3X will try and parse this region later down the line.
...
func (b *Block) values() cty.Value {
	...
	// Convert child blocks to list values for provider and module blocks.
	// Resource/data blocks are excluded because their child blocks (e.g., ingress, egress)
	// are handled by the evaluator's block expansion logic.
	if b.Type() == "provider" || b.Type() == "module" {
...
//
// The following resource residing in a module named "networking":
//
...
//			}
//
// Would have its FullName as module.networking.aws_instance.t3_standard
