...
// engine.Estimate cost retrieval. Without Evaluator the Blocks provided only have shallow information
...
func (e *Evaluator) collectModules() *Module {
	...
	if v := e.MissingVars(); len(v) > 0 {
		root.Warnings = append(root.Warnings, engine.NewDiagMissingVars(v...))
	}
...
func (e *Evaluator) loadModules(lastContext hcl.EvalContext) {
	...
	// NOTE: if a module uses a count that depends on another module's output, block expansion may be incorrect.
...
func ExpFunctions(baseDir string, logger zerolog.Logger) map[string]function.Function {
	fns := map[string]function.Function{
		...
		"c3xlog":     funcs.LogArgs(logger), // custom
		"c3xprint":   funcs.PrintArgs,       // custom
		"pow":              stdlib.PowFunc,
