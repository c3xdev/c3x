...
// mockedValForFunctionArg returns a type-appropriate mock value for function arguments.
// Functions that expect lists/tuples get an empty tuple, maps get an empty object,
// and everything else gets the default string mock.
func mockedValForFunctionArg(funcName string, defaultMock cty.Value) cty.Value {
	listFuncs := map[string]bool{
		"concat": true, "flatten": true, "compact": true, "distinct": true,
		"sort": true, "reverse": true, "coalescelist": true, "tolist": true,
		"toset": true, "setintersection": true, "setsubtract": true, "setunion": true,
		"chunklist": true, "slice": true, "keys": true, "values": true,
		"matchkeys": true, "zipmap": true, "split": true, "regexall": true,
	}
	mapFuncs := map[string]bool{
		"merge": true, "tomap": true, "transpose": true,
	}
	numberFuncs := map[string]bool{
		"length": true, "min": true, "max": true, "sum": true,
		"abs": true, "ceil": true, "floor": true, "parseint": true,
		"signum": true, "pow": true, "log": true,
	}

	if listFuncs[funcName] {
		return cty.EmptyTupleVal
	}
	if mapFuncs[funcName] {
		return cty.EmptyObjectVal
	}
	if numberFuncs[funcName] {
	...
	case *hclsyntax.FunctionCallExpr:
		...
		for i, exp := range t.Args {
			...
			for _, d := range diagnostics {
				if exp == d.Expression {
					argMock := mockedValForFunctionArg(t.Name, mockedVal)
...
// and their parents. C3X uses these references in resource evaluation to lookup connecting resource information.
...
func (attr *Attribute) referencesFromExpression(expression hcl.Expression) []*Reference {
	...
	case *hclsyntax.ObjectConsExpr:
		...
		return refs
	case *hclsyntax.ObjectConsKeyExpr:
		// Object keys are only treated as references if ForceNonLiteral is true (i.e.,
		// the key was wrapped in parentheses like (var.name) = value). Bare keys like
		// "aws.us-east-1" in provider maps are literal strings, not references, even
		// though they parse as multi-part traversals.
		if t.ForceNonLiteral {
