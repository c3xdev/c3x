...
// is outlined here: https://www.c3x.dev/config-file
type Project struct {
...
func DefaultConfig() *Settings {
	return &Settings{
		...
		DefaultPricingAPIEndpoint: "https://pricing.api.c3x.dev",
		PricingAPIEndpoint:        "",
		DashboardAPIEndpoint:      "https://dashboard.api.c3x.dev",
		DashboardEndpoint:         "https://dashboard.c3x.dev",
		...
		EventsDisabled: IsTest() || !IsEnvPresent("C3X_API_KEY"),
	}
}
...
// CachePath finds path which contains the .c3x directory. It traverses parent directories until a .c3x
// folder is found. If no .c3x folders exist then CachePath uses the current wd.
...
func (c *Settings) cachePath(dir string) string {
	for {
		cachePath := filepath.Join(dir, C3XDir)
...
func (c *Settings) loadEnvVars() error {
	err := envconfig.Process("C3X", c)
	if err != nil {
		return err
	}

	for _, project := range c.Projects {
		err = envconfig.Process("C3X", project)
		if err != nil {
			return err
		}
	}
...
func IsTest() bool {
	return os.Getenv("C3X_ENV") == "test" || strings.HasSuffix(os.Args[0], ".test")
}

func IsDev() bool {
	return os.Getenv("C3X_ENV") == "dev"
}

func loadDotEnv() error {
	if os.Getenv("C3X_DISABLE_ENVFILE") == "true" {
		return nil
	}
