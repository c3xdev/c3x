...
func (f *SettingsFileSpec) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	validationError := &YamlError{
		base: "config file is invalid, see https://c3x.dev/config-file for valid options",
	}
	...
	if !checkVersion(r.Version) {
		return &YamlError{
			base: "config file is invalid, see https://c3x.dev/config-file for file specification",
			errors: []error{
	...
	type fileSpecClone SettingsFileSpec
	var c fileSpecClone
	err = unmarshal(&c)
	if err != nil {
		return &YamlError{raw: ErrorInvalidConfigFile}
