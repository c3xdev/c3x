...
var (
	defaultC3XTmplName = "c3x.yml.tmpl"
)
...
// Parser is the representation of an initialized C3X template parser.
...
// NewParser returns a safely initialized C3X template parser, this builds the underlying template with the
...
func NewParser(repoDir string, variables Variables, config *settings.Settings) *Parser {
	absRepoDir, _ := filepath.Abs(repoDir)
	p := Parser{repoDir: absRepoDir, variables: variables, config: config}
	t := template.New(defaultC3XTmplName).Funcs(template.FuncMap{
		"base":         p.base,
		"stem":         p.stem,
		"ext":          p.ext,
		"lower":        p.lower,
...
// CompileFromFile writes an C3X config to io.Writer wr using the provided template path.
func (p *Parser) CompileFromFile(templatePath string, wr io.Writer) error {
	baseTemplate := p.template

	// if the template name is not `c3x.yml.tmpl` we need to change the template base
...
// Compile writes an C3X config to io.Writer wr using the provided template string.
...
func (p *Parser) pathExists(base, path string) bool {
	...
	// e.g. `/path/to/c3x` shouldn't match `/path/to/infra`.
