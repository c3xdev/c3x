	...
	c := &Session{
		...
		ContextValues: NewContextValues(
			map[string]interface{}{
				...
				"cliPlatform":           os.Getenv("C3X_CLI_PLATFORM"),
				"ciScript":              ciScript(),
				"ciPostCondition":       os.Getenv("C3X_CI_POST_CONDITION"),
				"ciPercentageThreshold": os.Getenv("C3X_CI_PERCENTAGE_THRESHOLD"),
			}),
		ModuleMutex: &intSync.KeyMutex{},
...
func (r *Session) EventEnv() map[string]interface{} {
	return r.EventEnvWithProjectContexts([]*ProjectSession{})
}
...
func ciScript() string {
	if IsEnvPresent("C3X_CI_IMAGE") {
		return "ci-image"
	} else if IsEnvPresent("C3X_GITHUB_ACTION") {
		return "c3x-github-action"
	} else if IsEnvPresent("C3X_CI_DIFF") {
		return "ci-diff"
	} else if IsEnvPresent("C3X_CI_ATLANTIS_DIFF") {
		return "ci-atlantis-diff"
	} else if IsEnvPresent("C3X_CI_JENKINS_DIFF") {
...
func ciPlatform() string {
	if os.Getenv("C3X_CI_PLATFORM") != "" {
		return os.Getenv("C3X_CI_PLATFORM")
	}
