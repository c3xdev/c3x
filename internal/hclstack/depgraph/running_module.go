...
func crossLinkDependencies(modules map[string]*runningModule, dependencyOrder DependencyOrder) (map[string]*runningModule, error) {
	for _, module := range modules {
		for _, dependency := range module.Module.Dependencies {
			...
			if !hasDependency {
				return modules, errtrace.WithStackTrace(DependencyNotFoundWhileCrossLinking{module, dependency})
			}
...
func (err DependencyFinishedWithError) ExitStatus() (int, error) {
	if exitCode, exitErr := shell.GetExitCode(err.Err); exitErr == nil {
		return exitCode, nil
	}
	return -1, err
}
