...
func GetDiagnosticsWriter(logger *logrus.Entry, parser *hclparse.Parser) hcl.DiagnosticWriter {
	termColor := terminal.IsTerminal(int(os.Stderr.Fd()))      //nolint:gosec // terminal fd conversion
	termWidth, _, err := terminal.GetSize(int(os.Stdout.Fd())) //nolint:gosec // terminal fd conversion
	if err != nil {
		termWidth = 80
	}
