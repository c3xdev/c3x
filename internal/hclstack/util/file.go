...
func FileOrData(maybePath string) (string, error) {
	...
	if IsFile(expandedMaybePath) {
		...
		return "", errtrace.WithStackTrace(PathIsNotFile{path: expandedMaybePath})
	}
...
func EnsureDirectory(path string) error {
	if FileExists(path) && IsFile(path) {
		return errtrace.WithStackTrace(PathIsNotDirectory{path})
	} else if !FileExists(path) {
		return errtrace.WithStackTrace(os.MkdirAll(path, 0700))
	}
	return nil
}
...
func ReadFileAsString(path string) (string, error) {
	...
	if err != nil {
		return "", errtrace.WithStackTraceAndPrefix(err, "Error reading file at path %s", path)
	}
...
// The problem is as follows: terragrunt copies the terraform source code first to "working directory" using modsource,
...
// to be cleaned in the "working directory" as well. Since we don't really know what can get copied by modsource,
