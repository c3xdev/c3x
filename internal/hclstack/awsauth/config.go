...
import (
	...
	"github.com/c3xdev/c3x/internal/hclstack/verutil"
...
// addUserAgent - Add grunt version to the user agent for AWS API calls.
var addUserAgent = request.NamedHandler{
	Name: "grunt.UserAgentHandler",
	Fn: request.MakeAddToUserAgentHandler(
		"grunt", verutil.GetVersion()),
}
...
func CreateAwsSessionFromConfig(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) (*session.Session, error) {
	...
	if err != nil {
		return nil, errtrace.WithStackTraceAndPrefix(err, "Error initializing session")
	}
	...
	iamRoleOptions := gruntOptions.IAMRoleOptions
	if config.RoleArn != "" {
...
func CreateAwsSession(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) (*session.Session, error) {
	...
	if config == nil {
		...
		sess, err = session.NewSessionWithOptions(sessionOptions)
		if err != nil {
			return nil, errtrace.WithStackTrace(err)
		}
		sess.Handlers.Build.PushFrontNamed(addUserAgent)
		if gruntOptions.IAMRoleOptions.RoleARN != "" {
			gruntOptions.Logger.Debugf("Assuming role %s", gruntOptions.IAMRoleOptions.RoleARN)
			sess.Config.Credentials = getSTSCredentialsFromIAMRoleOptions(sess, gruntOptions.IAMRoleOptions)
		}
	} else {
		sess, err = CreateAwsSessionFromConfig(config, gruntOptions)
		if err != nil {
			return nil, errtrace.WithStackTrace(err)
		}
	}
	...
	if _, err = sess.Config.Credentials.Get(); err != nil {
		...
		return nil, errtrace.WithStackTraceAndPrefix(err, "%s", msg)
	}
...
func AssumeIamRole(iamRoleOpts options.IAMRoleOptions) (*sts.Credentials, error) {
	...
	if err != nil {
		return nil, errtrace.WithStackTraceAndPrefix(err, "Error finding AWS credentials (did you set the AWS_ACCESS_KEY_ID and AWS_SECRET_ACCESS_KEY environment variables?)")
	}
...
func GetAWSCallerIdentity(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) (sts.GetCallerIdentityOutput, error) {
	sess, err := CreateAwsSession(config, gruntOptions)
	if err != nil {
		return sts.GetCallerIdentityOutput{}, errtrace.WithStackTrace(err)
	}
...
func ValidateAwsSession(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) error {
	// read the caller identity to check if the credentials are valid
	_, err := GetAWSCallerIdentity(config, gruntOptions)
	return err
}

// Get the AWS Partition of the current session configuration
func GetAWSPartition(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) (string, error) {
	identity, err := GetAWSCallerIdentity(config, gruntOptions)
	if err != nil {
...
func GetAWSAccountID(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) (string, error) {
	identity, err := GetAWSCallerIdentity(config, gruntOptions)
	if err != nil {
		return "", errtrace.WithStackTrace(err)
	}
...
func GetAWSIdentityArn(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) (string, error) {
	identity, err := GetAWSCallerIdentity(config, gruntOptions)
	if err != nil {
		return "", errtrace.WithStackTrace(err)
	}
...
func GetAWSUserID(config *AwsSessionConfig, gruntOptions *options.TerragruntOptions) (string, error) {
	identity, err := GetAWSCallerIdentity(config, gruntOptions)
	if err != nil {
		return "", errtrace.WithStackTrace(err)
	}
...
// we get back inside of gruntOptions.Env
func AssumeRoleAndUpdateEnvIfNecessary(gruntOptions *options.TerragruntOptions) error {
	iamRoleOpts := gruntOptions.IAMRoleOptions
	if iamRoleOpts.RoleARN == "" {
		return nil
	}

	gruntOptions.Logger.Debugf("Assuming IAM role %s with a session duration of %d seconds.", iamRoleOpts.RoleARN, iamRoleOpts.AssumeRoleDuration)
	creds, err := AssumeIamRole(iamRoleOpts)
	if err != nil {
		return err
	}

	gruntOptions.Env["AWS_ACCESS_KEY_ID"] = aws.StringValue(creds.AccessKeyId)
	gruntOptions.Env["AWS_SECRET_ACCESS_KEY"] = aws.StringValue(creds.SecretAccessKey)
	gruntOptions.Env["AWS_SESSION_TOKEN"] = aws.StringValue(creds.SessionToken)
	gruntOptions.Env["AWS_SECURITY_TOKEN"] = aws.StringValue(creds.SessionToken)
