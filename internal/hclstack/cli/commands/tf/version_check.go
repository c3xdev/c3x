...
// checkVersionConstraints validates terraform and terragrunt version constraints.
// It also resolves the terraform binary path and populates TerraformVersion and
// TerraformImplementation on the provided options.
...
func checkTerragruntVersionMeetsConstraint(currentVersion *version.Version, constraint string) error {
	...
	if !versionConstraint.Check(currentVersion) {
		return errtrace.WithStackTrace(InvalidTerragruntVersion{CurrentVersion: currentVersion, VersionConstraints: versionConstraint})
	}
...
func checkTerraformVersionMeetsConstraint(currentVersion *version.Version, constraint string) error {
	...
	if !versionConstraint.Check(currentVersion) {
		return errtrace.WithStackTrace(InvalidTerraformVersion{CurrentVersion: currentVersion, VersionConstraints: versionConstraint})
	}
...
func parseTerraformVersion(versionCommandOutput string) (*version.Version, error) {
	...
	if len(matches) != 3 {
		return nil, errtrace.WithStackTrace(InvalidTerraformVersionSyntax(versionCommandOutput))
	}
...
func parseTerraformImplementationType(versionCommandOutput string) (options.TerraformImplementationType, error) {
	...
	if len(matches) != 3 {
		return options.UnknownImpl, errtrace.WithStackTrace(InvalidTerraformVersionSyntax(versionCommandOutput))
	}
