...
import (
	...
	"github.com/c3xdev/c3x/internal/hclstack/cli/commands"
...
func DownloadTerraformSource(source string, terragruntOptions *options.TerragruntOptions, terragruntConfig *config.TerragruntConfig) (*options.TerragruntOptions, error) {
	terraformSource, err := terraform.NewSource(source, terragruntOptions.DownloadDir, terragruntOptions.WorkingDir, terragruntConfig.GenerateConfigs, terragruntOptions.Logger)
	if err != nil {
		return nil, err
	}
...
func downloadTerraformSourceIfNecessary(terraformSource *terraform.Source, terragruntOptions *options.TerragruntOptions, terragruntConfig *config.TerragruntConfig) error {
	...
	// https://github.com/c3xdev/c3x/internal/hclstack/issues/1921
...
// updateGetters returns the customized modsource interfaces that Terragrunt relies on. Specifically:
//   - Local file path getter is updated to copy the files instead of creating symlinks, which is what modsource defaults
//     to.
...
func updateGetters(terragruntConfig *config.TerragruntConfig) func(*getter.Client) error {
	return func(client *getter.Client) error {
		// We copy all the default getters from the modsource library, but replace the "file" getter. We shallow clone the
...
// Download the code from the Canonical Source URL into the Download Folder using the modsource library
func downloadSource(terraformSource *terraform.Source, terragruntOptions *options.TerragruntOptions, terragruntConfig *config.TerragruntConfig) error {
	...
	if err := getter.GetAny(context.Background(), terraformSource.DownloadDir, terraformSource.CanonicalSourceURL.String(), updateGetters(terragruntConfig)); err != nil {
		return errtrace.WithStackTrace(err)
	}
