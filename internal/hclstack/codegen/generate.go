...
func WriteToFile(terragruntOptions *options.TerragruntOptions, basePath string, config GenerateConfig) error {
	...
	if err := os.WriteFile(targetPath, []byte(contentsToWrite), 0600); err != nil {
...
func shouldContinueWithFileExists(terragruntOptions *options.TerragruntOptions, path string, ifExists GenerateConfigExists) (bool, error) {
	switch ifExists {
	case ExistsError:
		return false, errtrace.WithStackTrace(GenerateFileExistsError{path: path})
	case ExistsSkip:
	...
	default:
		...
		return false, errtrace.WithStackTrace(UnknownGenerateIfExistsVal{""})
	}
}
...
func GenerateConfigExistsFromString(val string) (GenerateConfigExists, error) {
	...
	return ExistsUnknown, errtrace.WithStackTrace(UnknownGenerateIfExistsVal{val: val})
}
