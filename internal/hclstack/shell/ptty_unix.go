...
import (
	...
	"golang.org/x/crypto/ssh/terminal" //nolint:staticcheck // migration to x/term is a separate task
...
func runCommandWithPTTY(terragruntOptions *options.TerragruntOptions, cmd *exec.Cmd, cmdStdout io.Writer, cmdStderr io.Writer) (err error) {
	...
	defer func() {
		if closeErr := pseudoTerminal.Close(); closeErr != nil {
			...
			if err == nil {
				err = errtrace.WithStackTrace(closeErr)
			}
		}
	}()
	if startErr != nil {
		return errtrace.WithStackTrace(startErr)
	}
	...
	oldState, setRawErr := terminal.MakeRaw(int(os.Stdin.Fd())) //nolint:gosec // terminal size conversion
	if setRawErr != nil {
		return errtrace.WithStackTrace(setRawErr)
	}
	defer func() {
		if restoreErr := terminal.Restore(int(os.Stdin.Fd()), oldState); restoreErr != nil { //nolint:gosec // terminal size conversion
			terragruntOptions.Logger.Errorf("Error restoring terminal state: %s", restoreErr)
			// Only overwrite the previous error if there was no error since this error has lower priority than any
			// errors in the main routine
			if err == nil {
				err = errtrace.WithStackTrace(restoreErr)
			}
		}
	}()
	...
	if copyStdoutErr != nil {
		return errtrace.WithStackTrace(copyStdoutErr)
	}
