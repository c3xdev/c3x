...
func RunShellCommandWithOutput(
	...
	return &cmdOutput, errtrace.WithStackTrace(err)
}
	...
	go func() {
		for {
			...
			case s := <-signalChannel:
				logger.Debugf("%s signal received. Gracefully shutting down... (it can take up to %v)", strings.Title(s.String()), signalForwardingDelay) //nolint:staticcheck // migration to x/text/cases is a separate task
