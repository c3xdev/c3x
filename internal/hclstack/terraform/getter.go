...
// RegistryGetter is a Getter (from modsource) implementation that will download from the terraform module
...
// GH issue: https://github.com/c3xdev/c3x/internal/hclstack/issues/1771
//
...
// GH issue: https://github.com/c3xdev/c3x/internal/hclstack/issues/1772
...
func (tfrGetter *RegistryGetter) ClientMode(ctx context.Context, u *url.URL) (getter.ClientMode, error) {
...
func (tfrGetter *RegistryGetter) Get(ctx context.Context, dstPath string, srcURL *url.URL) error {
	...
	if !hasVersion {
		return errtrace.WithStackTrace(MalformedRegistryURLErr{reason: "missing version query"})
	}
	if len(versionList) != 1 {
		return errtrace.WithStackTrace(MalformedRegistryURLErr{reason: "more than one version query"})
	}
...
// a single file.
func (tfrGetter *RegistryGetter) GetFile(ctx context.Context, dst string, src *url.URL) error {
	return errtrace.WithStackTrace(fmt.Errorf("GetFile is not implemented for the Terraform Registry Getter"))
}
...
func (tfrGetter *RegistryGetter) getSubdir(ctx context.Context, dstPath, sourceURL, subDir string) error {
	...
	if _, err := os.Stat(sourcePath); err != nil {
		...
		return errtrace.WithStackTrace(ModuleDownloadErr{sourceURL: sourceURL, details: details})
	}
...
func (tfrGetter *RegistryGetter) getModuleRegistryURLBasePath(ctx context.Context, domain string) (string, error) {
	...
	if err := json.Unmarshal(bodyData, &respJSON); err != nil {
		...
		return "", errtrace.WithStackTrace(ServiceDiscoveryErr{reason: reason})
	}
...
func (tfrGetter *RegistryGetter) getTerraformGetHeader(ctx context.Context, url url.URL) (string, error) {
	...
	if err != nil {
		...
		return "", errtrace.WithStackTrace(ModuleDownloadErr{sourceURL: url.String(), details: details})
	}
...
func (tfrGetter *RegistryGetter) httpGETAndGetResponse(ctx context.Context, getURL url.URL) ([]byte, *http.Header, error) {
	...
	if err != nil {
		return nil, nil, errtrace.WithStackTrace(err)
	}
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return nil, nil, errtrace.WithStackTrace(RegistryAPIErr{url: getURL.String(), statusCode: resp.StatusCode})
	}

	bodyData, err := io.ReadAll(resp.Body)
	return bodyData, &resp.Header, errtrace.WithStackTrace(err)
}
