...
import (
	...
	urlhelper "github.com/c3xdev/c3x/internal/modsource/helper/url"
...
func (terraformSource *Source) String() string {
	return fmt.Sprintf("Source{CanonicalSourceURL = %v, DownloadDir = %v, WorkingDir = %v, VersionFile = %v}", terraformSource.CanonicalSourceURL, terraformSource.DownloadDir, terraformSource.WorkingDir, terraformSource.VersionFile)
}
...
func (terraformSource Source) WriteVersionFile() error {
	...
	return errtrace.WithStackTrace(os.WriteFile(terraformSource.VersionFile, []byte(version), 0600))
}
...
func toSourceUrl(source string, workingDir string) (*url.URL, error) {
	// The modsource library is what Terraform's init command uses to download source URLs. Use that library to
	// parse the URL.
...
// Parse the given source URL into a URL struct. This method can handle source URLs that include modsource's "forced
...
// rest of the URL. This code is copied from the getForcedGetter method of modsource/get.go, as that method is not
// exported publicly.
