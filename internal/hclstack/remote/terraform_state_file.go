...
// NOTE: This uses direct JSON parsing rather than the Terraform SDK for state files, as the SDK is
...
func ParseTerraformStateFile(path string) (*TerraformState, error) {
	...
	if err != nil {
		return nil, errtrace.WithStackTrace(CantParseTerraformStateFile{Path: path, UnderlyingErr: err})
	}
