...
func validateGCSConfig(extendedConfig *ExtendedRemoteStateConfigGCS, terragruntOptions *options.TerragruntOptions) error {
	...
	if config.Bucket == "" {
		return errtrace.WithStackTrace(MissingRequiredGCSRemoteStateConfig("bucket"))
	}
...
func createGCSBucketIfNecessary(gcsClient *storage.Client, config *ExtendedRemoteStateConfigGCS, terragruntOptions *options.TerragruntOptions) error {
	if !DoesGCSBucketExist(gcsClient, &config.remoteStateConfigGCS) {
		...
		if config.Project == "" {
			return errtrace.WithStackTrace(MissingRequiredGCSRemoteStateConfig("project"))
		}

		// A location must be specified in order for terragrunt to automatically create a storage bucket.
		if config.Location == "" {
			return errtrace.WithStackTrace(MissingRequiredGCSRemoteStateConfig("location"))
		}
...
func DoesGCSBucketExist(gcsClient *storage.Client, config *RemoteStateConfigGCS) bool {
	...
	// NOTE: Bucket existence is checked via Attrs + listing, adapted from Google's integration tests.
	// See: https://github.com/GoogleCloudPlatform/google-cloud-go/blob/de879f7be552d57556875b8aaa383bce9396cc8c/storage/integration_test.go#L1231
