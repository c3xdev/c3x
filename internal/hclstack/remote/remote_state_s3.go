...
import (
	...
	"github.com/c3xdev/c3x/internal/hclstack/dynamodb"
...
func (c *ExtendedRemoteStateConfigS3) GetAwsSessionConfig() *awsauth.AwsSessionConfig {
	return &awsauth.AwsSessionConfig{
...
func (s3Initializer S3Initializer) NeedsInitialization(remoteState *RemoteState, existingBackend *TerraformBackend, terragruntOptions *options.TerragruntOptions) (bool, error) {
	...
	if err = awsauth.ValidateAwsSession(sessionConfig, terragruntOptions); err != nil {
		return false, err
	}
...
func validateS3Config(extendedConfig *ExtendedRemoteStateConfigS3, terragruntOptions *options.TerragruntOptions) error {
	...
	if config.Region == "" {
		return errtrace.WithStackTrace(MissingRequiredS3RemoteStateConfig("region"))
	}

	if config.Bucket == "" {
		return errtrace.WithStackTrace(MissingRequiredS3RemoteStateConfig("bucket"))
	}

	if config.Key == "" {
		return errtrace.WithStackTrace(MissingRequiredS3RemoteStateConfig("key"))
	}
...
func updateS3BucketIfNecessary(s3Client *s3.S3, config *ExtendedRemoteStateConfigS3, terragruntOptions *options.TerragruntOptions) error {
	if !DoesS3BucketExist(s3Client, &config.remoteStateConfigS3.Bucket) {
		...
		return errtrace.WithStackTrace(fmt.Errorf("remote state S3 bucket %s does not exist or you don't have permissions to access it", config.remoteStateConfigS3.Bucket))
	}
...
func checkIfVersioningEnabled(s3Client *s3.S3, config *RemoteStateConfigS3, terragruntOptions *options.TerragruntOptions) (bool, error) {
	...
	if err != nil {
		return false, errtrace.WithStackTrace(err)
	}
...
func WaitUntilS3BucketExists(s3Client *s3.S3, config *RemoteStateConfigS3, terragruntOptions *options.TerragruntOptions) error {
	...
	return errtrace.WithStackTrace(MaxRetriesWaitingForS3BucketExceeded(config.Bucket))
}
...
func EnableRootAccesstoS3Bucket(s3Client *s3.S3, config *ExtendedRemoteStateConfigS3, terragruntOptions *options.TerragruntOptions) error {
	...
	accountID, err := awsauth.GetAWSAccountID(config.GetAwsSessionConfig(), terragruntOptions)
	if err != nil {
		return errtrace.WithStackTrace(err)
	}

	partition, err := awsauth.GetAWSPartition(config.GetAwsSessionConfig(), terragruntOptions)
	if err != nil {
		return errtrace.WithStackTrace(err)
	}

	var policyInBucket awsauth.Policy
	...
	if policyOutput.Policy != nil {
		...
		policyInBucket, err = awsauth.UnmarshalPolicy(*policyOutput.Policy)
		if err != nil {
			return errtrace.WithStackTrace(err)
		}
	}
	...
	rootS3Policy := awsauth.Policy{
		Version: "2012-10-17",
		Statement: []awsauth.Statement{
			{
				Sid:    SidRootPolicy,
				Effect: "Allow",
				Action: "s3:*",
	...
	policy, err := awsauth.MarshalPolicy(rootS3Policy)
	if err != nil {
		return errtrace.WithStackTrace(err)
	}
...
func EnableEnforcedTLSAccesstoS3Bucket(s3Client *s3.S3, bucket string, config *ExtendedRemoteStateConfigS3, terragruntOptions *options.TerragruntOptions) error {
	...
	policy, err := awsauth.MarshalPolicy(tlsS3Policy)
	if err != nil {
		return errtrace.WithStackTrace(err)
	}
...
func waitUntilBucketHasAccessLoggingAcl(s3Client *s3.S3, bucket *string, terragruntOptions *options.TerragruntOptions) error {
	...
	return errtrace.WithStackTrace(MaxRetriesWaitingForS3ACLExceeded(aws.StringValue(bucket)))
}
...
func CreateS3Client(config *awsauth.AwsSessionConfig, terragruntOptions *options.TerragruntOptions) (*s3.S3, error) {
	session, err := awsauth.CreateAwsSession(config, terragruntOptions)
	if err != nil {
		return nil, err
	}
