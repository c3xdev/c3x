...
func RunTflintWithOpts(terragruntOptions *options.TerragruntOptions, terragruntConfig *config.TerragruntConfig, hook config.Hook) error {
	...
	if externalTfLint {
		...
		if err != nil {
			return errtrace.WithStackTrace(ErrorRunningTflint{args: initArgs})
		}
	} else {
	...
	if externalTfLint {
		...
		if err != nil {
			return errtrace.WithStackTrace(ErrorRunningTflint{args: args})
		}
		...
		case cmd.ExitCodeIssuesFound:
			return errtrace.WithStackTrace(IssuesFound{})
		case cmd.ExitCodeOK:
			terragruntOptions.Logger.Info("Tflint has run successfully. No issues found.")
		default:
			return errtrace.WithStackTrace(UnknownError{statusCode: statusCode})
		}
	}
	return nil
}
...
func findTflintConfigInProject(terragruntOptions *options.TerragruntOptions) (string, error) {
	...
	for i := 0; i < terragruntOptions.MaxFoldersToCheck; i++ {
		...
		if currentDir == previousDir {
			return "", errtrace.WithStackTrace(ConfigNotFound{cause: "Traversed all the day to the root"})
		}
	...
	return "", errtrace.WithStackTrace(ConfigNotFound{cause: fmt.Sprintf("Exceeded maximum folders to check (%d)", terragruntOptions.MaxFoldersToCheck)})
}
