...
import (
	...
	"github.com/c3xdev/c3x/internal/hclstack/awsauth"
	"github.com/c3xdev/c3x/internal/hclstack/options"
	"github.com/c3xdev/c3x/internal/hclstack/shell"
...
func CreateTerragruntEvalContext(
	...
	// https://github.com/c3xdev/c3x/internal/hclstack/blob/master/go.mod#L22
...
func getPathFromRepoRoot(trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (string, error) {
	...
	if err != nil {
		return "", errtrace.WithStackTrace(err)
	}
...
func parseGetEnvParameters(parameters []string) (EnvVar, error) {
	...
	default:
		return envVariable, errtrace.WithStackTrace(InvalidGetEnvParams{ActualNumParams: len(parameters), Example: `getEnv("<NAME>", "[DEFAULT]")`})
	}

	if envVariable.Name == "" {
		return envVariable, errtrace.WithStackTrace(InvalidEnvParamName{EnvVarName: parameters[0]})
	}
	return envVariable, nil
}

// runCommandCache - cache of evaluated `run_cmd` invocations
// see: https://github.com/c3xdev/c3x/internal/hclstack/issues/1427
...
func runCommand(args []string, trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (string, error) {
	if len(args) == 0 {
		return "", errtrace.WithStackTrace(EmptyStringNotAllowed("parameter to the run_cmd function"))
	}
...
func findInParentFolders(
	...
	if numParams > 2 {
		return "", errtrace.WithStackTrace(WrongNumberOfParams{Func: "find_in_parent_folders", Expected: "0, 1, or 2", Actual: numParams})
	}
	...
	for i := 0; i < terragruntOptions.MaxFoldersToCheck; i++ {
		...
		if currentDir == previousDir {
			...
			return "", errtrace.WithStackTrace(ParentFileNotFound{Path: terragruntOptions.TerragruntConfigPath, File: fileToFindStr, Cause: "Traversed all the way to the root"})
		}
	...
	return "", errtrace.WithStackTrace(ParentFileNotFound{Path: terragruntOptions.TerragruntConfigPath, File: fileToFindStr, Cause: fmt.Sprintf("Exceeded maximum folders to check (%d)", terragruntOptions.MaxFoldersToCheck)})
}
...
func getAWSAccountID(trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (string, error) {
	accountID, err := awsauth.GetAWSAccountID(nil, terragruntOptions)
	if err == nil {
...
func getAWSCallerIdentityARN(trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (string, error) {
	identityARN, err := awsauth.GetAWSIdentityArn(nil, terragruntOptions)
	if err == nil {
...
func getAWSCallerIdentityUserID(trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (string, error) {
	userID, err := awsauth.GetAWSUserID(nil, terragruntOptions)
	if err == nil {
		return userID, nil
	}
	return "", err
...
func readTerragruntConfig(configPath string, defaultVal *cty.Value, terragruntOptions *options.TerragruntOptions) (cty.Value, error) {
	...
	if !targetConfigFileExists && defaultVal == nil {
		return cty.NilVal, errtrace.WithStackTrace(TerragruntConfigNotFound{Path: targetConfig})
	...
	for i := 0; i < len(config.TerragruntDependencies); i++ {
		_ = config.TerragruntDependencies[i].setRenderedOutputs(targetOptions)
	}
	...
	return function.New(&function.Spec{
		...
		Impl: func(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			if numParams == 0 || numParams > 2 {
				return cty.NilVal, errtrace.WithStackTrace(WrongNumberOfParams{Func: "read_terragrunt_config", Expected: "1 or 2", Actual: numParams})
			}
...
func GetTerragruntSourceForModule(sourcePath string, modulePath string, moduleTerragruntConfig *TerragruntConfig) (string, error) {
	...
	// use modsource to split the module source string into a valid URL and subdirectory (if // is present)
	...
	if moduleUrl == "" && moduleSubdir == "" {
		return "", errtrace.WithStackTrace(InvalidSourceUrl{ModulePath: modulePath, ModuleSourceUrl: *moduleTerragruntConfig.Terraform.Source, TerragruntSource: sourcePath})
	}
...
func getModulePathFromSourceUrl(sourceUrl string) (string, error) {
	...
	if len(matches) != 2 {
		return "", errtrace.WithStackTrace(ErrorParsingModulePath{ModuleSourceUrl: sourceUrl})
	}
...
func sopsDecryptFile(params []string, trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (string, error) {
	...
	if numParams != 1 {
		return "", errtrace.WithStackTrace(WrongNumberOfParams{Func: "sops_decrypt_file", Expected: "1", Actual: numParams})
	}
	...
	return "", errtrace.WithStackTrace(InvalidSopsFormat{SourceFilePath: sourceFile})
}
...
func getSelectedIncludeBlock(trackInclude TrackInclude, params []string) (*IncludeConfig, error) {
	...
	numParams := len(params)
	if numParams != 1 {
		return nil, errtrace.WithStackTrace(WrongNumberOfParams{Func: "path_relative_from_include", Expected: "1", Actual: numParams})
	}

	importName := params[0]
	imported, hasKey := importMap[importName]
	if !hasKey {
		return nil, errtrace.WithStackTrace(InvalidIncludeKey{name: importName})
	}
	return &imported, nil
}
...
func startsWith(args []string, trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (bool, error) {
	if len(args) == 0 {
		return false, errtrace.WithStackTrace(EmptyStringNotAllowed("parameter to the startswith function"))
	}
	str := args[0]
	prefix := args[1]
...
func endsWith(args []string, trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (bool, error) {
	if len(args) == 0 {
		return false, errtrace.WithStackTrace(EmptyStringNotAllowed("parameter to the endswith function"))
	}
	str := args[0]
	suffix := args[1]
...
func timeCmp(args []string, trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (int64, error) {
	if len(args) != 2 {
		return 0, errtrace.WithStackTrace(fmt.Errorf("function can take only two parameters: timestamp_a and timestamp_b"))
	}

	tsA, err := util.ParseTimestamp(args[0])
	if err != nil {
		return 0, errtrace.WithStackTrace(fmt.Errorf("could not parse first parameter %q: %w", args[0], err))
	}
	tsB, err := util.ParseTimestamp(args[1])
	if err != nil {
		return 0, errtrace.WithStackTrace(fmt.Errorf("could not parse second parameter %q: %w", args[1], err))
	}
...
func strContains(args []string, trackInclude *TrackInclude, terragruntOptions *options.TerragruntOptions) (bool, error) {
	if len(args) == 0 {
		return false, errtrace.WithStackTrace(EmptyStringNotAllowed("parameter to the strcontains function"))
	}
	str := args[0]
	substr := args[1]
