...
func evaluateLocalsBlock(
	...
	if diags.HasErrors() {
		_ = diagsWriter.WriteDiagnostics(diags)
		return nil, errtrace.WithStackTrace(diags)
	}
	if localsBlock == nil {
	...
	for iterations := 0; len(locals) > 0 && evaluated; iterations++ {
		if iterations > MaxIter {
			...
			return nil, errtrace.WithStackTrace(MaxIterError{})
		}
	...
	if len(locals) > 0 {
		...
		return nil, errtrace.WithStackTrace(CouldNotEvaluateAllLocalsError{})
	}
...
func attemptEvaluateLocals(
	...
	for _, local := range locals {
		...
		if localEvaluated {
			...
			if diags.HasErrors() {
				_ = diagsWriter.WriteDiagnostics(diags)
				return nil, evaluatedLocals, false, errtrace.WithStackTrace(diags)
			}
...
func canEvaluate(
	...
	for _, var_ := range vars {
		...
		if var_.IsRelative() {
			reason := "You've reached an impossible condition and is almost certainly a bug in C3X. Please open an issue at github.com/c3xdev/c3x with this message and the contents of your terragrunt.hcl file that caused this."
			return false, reason
		}
...
func (err MaxIterError) Error() string {
	return "Maximum iterations reached in attempting to evaluate locals. This is most likely a bug in Terragrunt. Please file an issue on the project: https://github.com/c3xdev/c3x/internal/hclstack/issues"
}
