...
func parseIncludedConfig(
	...
	if includedConfig.Path == "" {
		return nil, errtrace.WithStackTrace(IncludedConfigMissingPath(terragruntOptions.TerragruntConfigPath))
	}
...
func handleInclude(
	...
	if trackInclude == nil {
		return nil, fmt.Errorf("You reached an impossible condition. This is most likely a bug in C3X. Please open an issue at github.com/c3xdev/c3x with this error message. Code: HANDLE_INCLUDE_NIL_INCLUDE_CONFIG")
	}
	...
	for i := len(includeList) - 1; i >= 0; i-- {
		...
		case DeepMerge:
			...
			baseConfig = parsedIncludeConfig
		default:
			return nil, fmt.Errorf("You reached an impossible condition. This is most likely a bug in C3X. Please open an issue at github.com/c3xdev/c3x with this error message. Code: UNKNOWN_MERGE_STRATEGY_%s", mergeStrategy)
		}
	}
...
func handleIncludePartial(
	...
	if trackInclude == nil {
		return nil, fmt.Errorf("You reached an impossible condition. This is most likely a bug in C3X. Please open an issue at github.com/c3xdev/c3x with this error message. Code: HANDLE_INCLUDE_PARTIAL_NIL_INCLUDE_CONFIG")
	}
	...
	for i := len(includeList) - 1; i >= 0; i-- {
		...
		case DeepMerge:
			...
			baseConfig = parsedIncludeConfig
		default:
			return nil, fmt.Errorf("You reached an impossible condition. This is most likely a bug in C3X. Please open an issue at github.com/c3xdev/c3x with this error message. Code: UNKNOWN_MERGE_STRATEGY_%s_PARTIAL", mergeStrategy)
		}
	}
	return baseConfig, nil
}
...
func handleIncludeForDependency(
	...
	if trackInclude == nil {
		return nil, fmt.Errorf("You reached an impossible condition. This is most likely a bug in C3X. Please open an issue at github.com/c3xdev/c3x with this error message. Code: HANDLE_INCLUDE_DEPENDENCY_NIL_INCLUDE_CONFIG")
	}
	// We merge in the include blocks in reverse order here. The expectation is that the bottom most elements override
	// those in earlier includes, so we need to merge bottom up instead of top down to ensure this.
	includeList := trackInclude.CurrentList
	...
	for i := len(includeList) - 1; i >= 0; i-- {
		...
		case DeepMerge:
			...
			baseDependencyBlock = mergedDependencyBlock
		default:
			return nil, fmt.Errorf("You reached an impossible condition. This is most likely a bug in C3X. Please open an issue at github.com/c3xdev/c3x with this error message. Code: UNKNOWN_MERGE_STRATEGY_%s_DEPENDENCY", mergeStrategy)
		}
	}
...
func (conf *TerragruntConfig) Merge(sourceConfig *TerragruntConfig, terragruntOptions *options.TerragruntOptions) error {
	...
	if sourceConfig.DownloadDir != "" {
		conf.DownloadDir = sourceConfig.DownloadDir
	}

	if sourceConfig.IamRole != "" {
		conf.IamRole = sourceConfig.IamRole
	}

	if sourceConfig.IamAssumeRoleDuration != nil {
		conf.IamAssumeRoleDuration = sourceConfig.IamAssumeRoleDuration
	}

	if sourceConfig.TerraformVersionConstraint != "" {
		conf.TerraformVersionConstraint = sourceConfig.TerraformVersionConstraint
	}

	if sourceConfig.TerraformBinary != "" {
		conf.TerraformBinary = sourceConfig.TerraformBinary
	}

	if sourceConfig.PreventDestroy != nil {
		conf.PreventDestroy = sourceConfig.PreventDestroy
	}

	if sourceConfig.RetryMaxAttempts != nil {
		conf.RetryMaxAttempts = sourceConfig.RetryMaxAttempts
	}

	if sourceConfig.RetrySleepIntervalSec != nil {
		conf.RetrySleepIntervalSec = sourceConfig.RetrySleepIntervalSec
	}

	if sourceConfig.TerragruntVersionConstraint != "" {
		conf.TerragruntVersionConstraint = sourceConfig.TerragruntVersionConstraint
	}

	// Skip has to be set specifically in each file that should be skipped
	conf.Skip = sourceConfig.Skip

	if sourceConfig.RemoteState != nil {
		conf.RemoteState = sourceConfig.RemoteState
	}

	if sourceConfig.Terraform != nil {
		if conf.Terraform == nil {
			conf.Terraform = sourceConfig.Terraform
		} else {
			if sourceConfig.Terraform.Source != nil {
				conf.Terraform.Source = sourceConfig.Terraform.Source
			}
			mergeByName(terragruntOptions, sourceConfig.Terraform.ExtraArgs, &conf.Terraform.ExtraArgs)

			mergeByName(terragruntOptions, sourceConfig.Terraform.BeforeHooks, &conf.Terraform.BeforeHooks)
			mergeByName(terragruntOptions, sourceConfig.Terraform.AfterHooks, &conf.Terraform.AfterHooks)
			mergeByName(terragruntOptions, sourceConfig.Terraform.ErrorHooks, &conf.Terraform.ErrorHooks)
		}
	}

	// Dependency blocks are shallow merged by name
	conf.TerragruntDependencies = mergeDependencyBlocks(conf.TerragruntDependencies, sourceConfig.TerragruntDependencies)
	...
	if sourceConfig.Dependencies != nil {
		if conf.Dependencies == nil {
			conf.Dependencies = sourceConfig.Dependencies
		} else {
			conf.Dependencies.Merge(sourceConfig.Dependencies)
		}
	}

	if sourceConfig.RetryableErrors != nil {
		conf.RetryableErrors = sourceConfig.RetryableErrors
	}
	...
	// C3X, and we don't want to fail the merge.
	//
	// err := validateGenerateConfigs(&sourceConfig.GenerateConfigs, &conf.GenerateConfigs)
	// if err != nil {
	//	return err
	// }

	for key, val := range sourceConfig.GenerateConfigs {
		conf.GenerateConfigs[key] = val
	}

	if sourceConfig.Inputs != nil {
		conf.Inputs = mergeInputs(sourceConfig.Inputs, conf.Inputs)
	}

	copyFieldsMetadata(sourceConfig, conf)
...
func (conf *TerragruntConfig) DeepMerge(sourceConfig *TerragruntConfig, terragruntOptions *options.TerragruntOptions) error {
	...
	if sourceConfig.Dependencies != nil {
		...
		if conf.Dependencies != nil {
			...
			// Fix for issue: https://github.com/c3xdev/c3x/internal/hclstack/issues/1900
			targetPathMap := fetchDependencyPaths(conf)
			...
			// if TerragruntDependencies will be empty, all conf.Dependencies.Paths will be copied to resultModuleDependencies.Paths
			for _, dependencyPath := range conf.Dependencies.Paths {
				var addPath = true
		...
		conf.Dependencies = resultModuleDependencies
	}

	// Dependency blocks are deep merged by name
	mergedDeps, err := deepMergeDependencyBlocks(conf.TerragruntDependencies, sourceConfig.TerragruntDependencies)
	if err != nil {
		return err
	}
	conf.TerragruntDependencies = mergedDeps

	if sourceConfig.RetryableErrors != nil {
		conf.RetryableErrors = append(conf.RetryableErrors, sourceConfig.RetryableErrors...)
	}
	...
	if sourceConfig.Terraform != nil {
		if conf.Terraform == nil {
			...
			if sourceConfig.Terraform.IncludeInCopy != nil {
				srcList := append([]string{}, *sourceConfig.Terraform.IncludeInCopy...)
				if conf.Terraform.IncludeInCopy != nil {
					targetList := *conf.Terraform.IncludeInCopy
					srcList = append(srcList, targetList...)
				}
				conf.Terraform.IncludeInCopy = &srcList
			}
	...
	if sourceConfig.Inputs != nil {
		mergedInputs, err := deepMergeInputs(sourceConfig.Inputs, conf.Inputs)
		if err != nil {
			return err
		}
		conf.Inputs = mergedInputs
	}
...
func deepMergeInputs(childInputs map[string]interface{}, parentInputs map[string]interface{}) (map[string]interface{}, error) {
	...
	return out, errtrace.WithStackTrace(err)
}

// mergeByName merges child items into parent items by name.
// If a child has the same name as a parent, the child overrides.
// Otherwise the child is appended.
func mergeByName[T named](terragruntOptions *options.TerragruntOptions, children []T, parent *[]T) {
	result := *parent
	for _, child := range children {
		idx := getIndexByName(result, child.GetName())
		if idx != -1 {
			terragruntOptions.Logger.Debugf("'%v' from child overriding parent", child.GetName())
			result[idx] = child
		} else {
...
func getTrackInclude(
	...
	if hasInclude && includeFromChild != nil {
		...
		err := errtrace.WithStackTrace(TooManyLevelsOfInheritance{
...
func updateBareIncludeBlock(file *hcl.File, filename string) ([]byte, bool, error) {
	...
	if diags.HasErrors() {
		return nil, false, errtrace.WithStackTrace(diags)
	}
	...
	for _, block := range hclFile.Body().Blocks() {
		if block.Type() == "include" && len(block.Labels()) == 0 {
			if codeWasUpdated {
				return nil, false, errtrace.WithStackTrace(MultipleBareIncludeBlocksErr{})
			}
...
func updateBareIncludeBlockJSON(fileBytes []byte) ([]byte, bool, error) {
	...
	if err := json.Unmarshal(fileBytes, &parsed); err != nil {
		return nil, false, errtrace.WithStackTrace(err)
	}
	...
	return nil, false, errtrace.WithStackTrace(IncludeIsNotABlockErr{parsed: includeBlock})
}
...
func updateSingleBareIncludeInParsedJSON(parsed map[string]interface{}, newVal interface{}) ([]byte, bool, error) {
	...
	return updatedBytes, true, errtrace.WithStackTrace(err)
}
