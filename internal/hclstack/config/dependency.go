...
import (
	...
	"github.com/c3xdev/c3x/internal/hclstack/remote"
	"github.com/c3xdev/c3x/internal/hclstack/shell"
	"github.com/c3xdev/c3x/internal/hclstack/terraform"
...
func (dependencyConfig *Dependency) DeepMerge(sourceDepConfig Dependency) error {
	if sourceDepConfig.ConfigPath != "" {
		dependencyConfig.ConfigPath = sourceDepConfig.ConfigPath
	}

	if sourceDepConfig.Enabled != nil {
		dependencyConfig.Enabled = sourceDepConfig.Enabled
	}

	if sourceDepConfig.SkipOutputs != nil {
		dependencyConfig.SkipOutputs = sourceDepConfig.SkipOutputs
	}

	if sourceDepConfig.MockOutputs != nil {
		if dependencyConfig.MockOutputs == nil {
			dependencyConfig.MockOutputs = sourceDepConfig.MockOutputs
		} else {
			newMockOutputs, err := deepMergeCtyMaps(*dependencyConfig.MockOutputs, *sourceDepConfig.MockOutputs)
			if err != nil {
				return err
			}
			dependencyConfig.MockOutputs = newMockOutputs
		}
	}

	if sourceDepConfig.MockOutputsAllowedTerraformCommands != nil {
		if dependencyConfig.MockOutputsAllowedTerraformCommands == nil {
			dependencyConfig.MockOutputsAllowedTerraformCommands = sourceDepConfig.MockOutputsAllowedTerraformCommands
		} else {
			mergedCmds := append(*dependencyConfig.MockOutputsAllowedTerraformCommands, *sourceDepConfig.MockOutputsAllowedTerraformCommands...)
			dependencyConfig.MockOutputsAllowedTerraformCommands = &mergedCmds
		}
	}
...
// NOTE: Dependency blocks from included configs are not currently imported to avoid cyclic dependency issues.
...
func checkForDependencyBlockCyclesUsingDFS(
	...
	if util.ListContainsElement(*currentTraversalPaths, currentConfigPath) {
		return errtrace.WithStackTrace(DependencyCycle(append(*currentTraversalPaths, currentConfigPath)))
	}
...
func dependencyBlocksToCtyValue(dependencyConfigs []Dependency, terragruntOptions *options.TerragruntOptions) (*cty.Value, error) {
	...
	for _, dependencyConfig := range dependencyConfigs {
		...
		dependencyErrGroup.Go(func() (err error) {
			defer func() {
				if r := recover(); r != nil {
					err = errtrace.WithStackTrace(fmt.Errorf("%v", r))
				}
			}()
	...
	return &convertedOutput, errtrace.WithStackTrace(err)
}
...
func getTerragruntOutputIfAppliedElseConfiguredDefault(dependencyConfig Dependency, terragruntOptions *options.TerragruntOptions) (*cty.Value, error) {
	...
	if dependencyConfig.shouldGetOutputs() {
		...
		if !isEmpty && dependencyConfig.shouldMergeMockOutputsWithState(terragruntOptions) && dependencyConfig.MockOutputs != nil {
			...
			default:
				return nil, errtrace.WithStackTrace(InvalidMergeStrategyType(mockMergeStrategy))
			}
...
func getTerragruntOutput(dependencyConfig Dependency, terragruntOptions *options.TerragruntOptions) (*cty.Value, bool, error) {
	...
	if !util.FileExists(targetConfig) {
		return nil, true, errtrace.WithStackTrace(DependencyConfigNotFound{Path: targetConfig})
	}
	...
	if err != nil {
		...
		jsonBytes, err = json.Marshal(dependencyConfig.MockOutputs) //nolint:staticcheck // cty.Value has custom JSON marshaling via GobEncode
		if err != nil {
			return nil, true, err
		}
	}
	...
	return &convertedOutput, isEmpty, errtrace.WithStackTrace(err)
}
...
func getOutputJsonWithCaching(targetConfig string, terragruntOptions *options.TerragruntOptions) ([]byte, error) {
	...
	// Related issues: https://github.com/c3xdev/c3x/internal/hclstack/issues/2233 https://github.com/hashicorp/terraform-provider-aws/issues/23620
...
func cloneTerragruntOptionsForDependency(terragruntOptions *options.TerragruntOptions, targetConfig string) *options.TerragruntOptions {
	...
	// https://github.com/c3xdev/c3x/internal/hclstack/issues/1853#issuecomment-940102676
...
func setupTerragruntOptionsForBareTerraform(originalOptions *options.TerragruntOptions, workingDir string, configPath string, iamRoleOpts options.IAMRoleOptions) (*options.TerragruntOptions, error) {
	...
	if err := awsauth.AssumeRoleAndUpdateEnvIfNecessary(targetTGOptions); err != nil {
		return nil, err
	}
...
func terraformOutputJsonToCtyValueMap(targetConfig string, jsonBytes []byte) (map[string]cty.Value, error) {
	...
	if err != nil {
		return nil, errtrace.WithStackTrace(TerragruntOutputParsingError{Path: targetConfig, Err: err})
	}
...
func runTerraformInitForDependencyOutput(terragruntOptions *options.TerragruntOptions, workingDir string, targetConfig string) {
	...
	if err != nil {
		...
		terragruntOptions.Logger.Debugf("%s", stderr.String())
	}
}
