...
func ComputeDiff(past []*Estimate, current []*Estimate) []*Estimate {
	...
	pastRMap := make(map[string]*Estimate)
	fillResourcesMap(pastRMap, "", past)
	currentRMap := make(map[string]*Estimate)
	fillResourcesMap(currentRMap, "", current)

	diff := make([]*Estimate, 0)
...
// their key.
func diffResourcesByKey(resourceKey string, pastResMap, currentResMap map[string]*Estimate) (bool, *Estimate) {
...
func diffCostComponentsByResource(pastResource, currentResource *Estimate) (bool, []*LineItem) {
	result := make([]*LineItem, 0)
	changed := false

	remainingCostComponents := map[string]*LineItem{}
	...
	for _, pastCostComponent := range pastResource.CostComponents {
		var currentCostComponent *LineItem
...
func diffCostComponents(past *LineItem, current *LineItem) (bool, *LineItem) {
...
func findMatchingCostComponent(costComponents []*LineItem, name string) *LineItem {
...
func fillResourcesMap(resourcesMap map[string]*Estimate, rootKey string, resources []*Estimate) {
