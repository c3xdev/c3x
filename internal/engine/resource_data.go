...
func NewResourceData(resourceType string, providerName string, address string, tags *map[string]string, rawValues gjson.Result) *ResourceSpec {
	return &ResourceSpec{
		...
		Tags:            tags,
		RawValues:       rawValues,
		ReferencesMap:   make(map[string][]*ResourceSpec),
		CFResource:      nil,
...
func (d *ResourceSpec) AddReference(k string, reference *ResourceSpec, reverseRefAttrs []string) {
	...
	if _, ok := d.ReferencesMap[key]; !ok {
		d.ReferencesMap[key] = make([]*ResourceSpec, 0)
	}
	...
	for _, attr := range reverseRefAttrs {
		if attr == reverseRefKey {
			if _, ok := reference.ReferencesMap[reverseRefKey]; !ok {
				reference.ReferencesMap[reverseRefKey] = make([]*ResourceSpec, 0)
			}
...
func AddRawValue(r gjson.Result, key string, v interface{}) gjson.Result {
	...
	if err := json.Unmarshal([]byte(r.Raw), &j); err != nil {
		j = make(map[string]interface{})
	}
