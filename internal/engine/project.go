...
import (
	...
	"github.com/c3xdev/c3x/internal/util"
...
const (
	...
	// Diags for c3x cloud issues
	diagRunQuotaExceeded = 401
)

// Diagnostic holds information about all diagnostics associated with a project.
...
func newDiag(code int, message string, isError bool, data interface{}, err error) *Diagnostic {
...
func DetectProjectMetadata(path string) *WorkspaceMeta {
	vcsSubPath := os.Getenv("C3X_VCS_SUB_PATH")
	terraformWorkspace := os.Getenv("C3X_TERRAFORM_WORKSPACE")
...
func (m *WorkspaceMeta) AddError(err error) {
	...
	m.Errors = append(m.Errors, &Diagnostic{Message: err.Error()})
}
...
// Workspace contains the existing, planned state of
...
func (p *Workspace) AllResources() []*Estimate {
	...
	resources := make([]*Estimate, 0, len(m))
	for r := range m {
...
func (p *Workspace) AllPartialResources() []*UnpricedEntry {
	...
	resources := make([]*UnpricedEntry, 0, len(m))
	for r := range m {
		resources = append(resources, r)
	}
...
func (p *Workspace) BuildResources(usageMap ConsumptionMap) {
	pastResources := make([]*Estimate, 0, len(p.PartialPastResources))
	resources := make([]*Estimate, 0, len(p.PartialResources))

	seen := make(map[*UnpricedEntry]*Estimate)
