...
func (u *ConsumptionProfile) Copy() *ConsumptionProfile {
	if u == nil {
		return nil
	}

	c := &ConsumptionProfile{
		Address:    u.Address,
		Attributes: map[string]gjson.Result{},
	}
...
func (u *ConsumptionProfile) Merge(other *ConsumptionProfile) *ConsumptionProfile {
	if u == nil {
		if other != nil {
			return other.Copy()
		}
...
// ConsumptionMap is a map of address to UsageData built from a usage file.
// ConsumptionMap is a standalone type so that we can do more involved matching functionality.
...
func NewUsageMapFromInterface(m map[string]interface{}) ConsumptionMap {
	data := make(map[string]*ConsumptionProfile)
...
func NewUsageMap(data map[string]*ConsumptionProfile) ConsumptionMap {
	...
	return ConsumptionMap{data: data, wildcards: keys}
}
...
func mergeUsage(dst *ConsumptionProfile, src *ConsumptionProfile) {
