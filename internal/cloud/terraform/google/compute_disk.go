...
func getComputeDiskRegistryItem() *engine.CatalogEntry {
...
func newComputeDisk(d *engine.ResourceSpec) engine.CatalogItem {
...
func computeDiskSize(d *engine.ResourceSpec) float64 {
...
func computeImageDiskSize(d *engine.ResourceSpec) float64 {
...
func computeSnapshotDiskSize(d *engine.ResourceSpec) float64 {
	if len(d.References("source_disk")) > 0 {
		return computeDiskSize(d.References("source_disk")[0])
	}
...
func computeIOPS(d *engine.ResourceSpec, diskType string, diskSize float64) int64 {
	if diskType == "pd-extreme" || diskType == "hyperdisk-extreme" {
