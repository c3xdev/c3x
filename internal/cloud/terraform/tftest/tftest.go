...
var (
	pluginCache = filepath.Join(settings.RootDir(), ".test_cache/terraform_plugins")
	initCache   = filepath.Join(settings.RootDir(), ".test_cache/terraform_init")
	once        sync.Once
)
...
func ResourceTestsForTerraformProject(t *testing.T, tfProject TerraformProject, usage engine.ConsumptionMap, checks []testutil.ResourceCheck, ctxOptions ...func(ctx *settings.Session)) {
	...
	t.Run("HCL Graph", func(t *testing.T) {
		ctxOptions = append(ctxOptions, func(ctx *settings.Session) {
...
func resourceTestsForTfProject(t *testing.T, pName string, tfProject TerraformProject, usage engine.ConsumptionMap, checks []testutil.ResourceCheck, ctxOptions ...func(ctx *settings.Session)) {
	t.Helper()

	runCtx, err := settings.NewRunContextFromEnv(context.Background())
...
func goldenFileResourceTestWithOpts(t *testing.T, pName string, testName string, options *GoldenFileOptions, ctxOptions ...func(ctx *settings.Session)) {
	t.Helper()
	...
	var usageData engine.ConsumptionMap
	...
	actual, err := render.ToTable(r, opts)
	require.NoError(t, err)
...
func loadResources(t *testing.T, pName string, tfProject TerraformProject, runCtx *settings.Session, usageData engine.ConsumptionMap) []*engine.Workspace {
	t.Helper()

	tfdir := createTerraformProject(t, tfProject)
	runCtx.Config.RootPath = tfdir
	var provider engine.Vendor
	if pName == "hcl" {
		provider = newHCLProvider(t, runCtx, tfdir)
	} else {
		provider = terraform.NewDirProvider(settings.NewProjectContext(runCtx, &settings.Project{
			Path:     tfdir,
	...
	for _, project := range projects {
		...
		project.BuildResources(engine.ConsumptionMap{})
	}
...
func RunCostCalculations(runCtx *settings.Session, projects []*engine.Workspace) ([]*engine.Workspace, error) {
	pf := pricing.NewPriceFetcher(runCtx, true)
	for _, project := range projects {
...
func goldenFileSyncTest(t *testing.T, pName, testName string, ctxOptions ...func(ctx *settings.Session)) {
	runCtx, err := settings.NewRunContextFromEnv(context.Background())
	require.NoError(t, err)
	...
	projectCtx := settings.NewProjectContext(runCtx, &settings.Project{}, nil)

	usageFilePath := filepath.Join("testdata", testName, testName+"_existing_usage.yml")
	projects := loadResources(t, pName, tfProject, runCtx, engine.ConsumptionMap{})
...
func newHCLProvider(t *testing.T, runCtx *settings.Session, tfdir string) *terraform.HCLLoader {
	t.Helper()

	projectCtx := settings.NewProjectContext(runCtx, &settings.Project{
		Path:     tfdir,
		Metadata: runCtx.Config.Projects[0].Metadata,
	}, nil)
