...
var (
	...
	moduleCacheRegex = regexp.MustCompile(`(?:.+)?\.c3x/terraform_modules/[^/]+/(.+)`)
)
...
// NewHCLProvider returns a HCLProvider with a hcl.Parser initialised using the settings.ProjectSession.
...
func (p *HCLLoader) Context() *settings.ProjectSession { return p.ctx }
...
func (p *HCLLoader) Type() string        { return "terraform_dir" }
func (p *HCLLoader) DisplayType() string { return "Terraform" }
...
// The PlanJSONProvider uses this shallow representation to actually load C3X resources.
func (p *HCLLoader) LoadResources(usage engine.ConsumptionMap) ([]*engine.Workspace, error) {
	...
	loadPlanTimer.Stop()
	if j.Error != nil {
		return []*engine.Workspace{p.newProject(j)}, nil
	}
	...
	parseJSONTimer.Stop()
	if err != nil {
		project.Metadata.AddError(engine.NewDiagJSONParsingFailure(err))
...
func (p *HCLLoader) newProject(parsed HCLProject) *engine.Workspace {
	metadata := engine.DetectProjectMetadata(parsed.Module.RootPath)
	metadata.Type = p.Type()
	p.AddMetadata(metadata)

	if parsed.Error != nil {
		metadata.AddError(engine.NewDiagModuleEvaluationFailure(parsed.Error))
	}
...
func (p *HCLLoader) printWarning(warning *engine.Diagnostic) {
...
func (p *HCLLoader) newPlanSchema() {
	p.schema = &PlanSchema{
		FormatVersion:    "1.0",
		...
		C3XResourceChanges: []ResourceChangesJSON{},
...
func (p *HCLLoader) modulesToPlanJSON(rootModule *hcl.Module) ([]byte, error) {
	p.newPlanSchema()
	if rootModule.ProviderConstraints != nil {
		p.schema.C3XProviderConstraints = *rootModule.ProviderConstraints
	}
...
func (p *HCLLoader) marshalModule(module *hcl.Module) ModuleOut {
	...
	for _, block := range module.Blocks {
		if block.Type() == "resource" {
			...
			p.schema.C3XResourceChanges = append(p.schema.C3XResourceChanges, out.Changes)
		}
	}
...
func (p *HCLLoader) getResourceOutput(block *hcl.Block, moduleSourceURL string) ResourceOutput {
...
func (p *HCLLoader) marshalProviderBlock(block *hcl.Block) string {
...
func (p *HCLLoader) marshalAWSDefaultTagsBlock(providerBlock *hcl.Block) map[string]interface{} {
...
func (p *HCLLoader) marshalGoogleDefaultTagsBlock(providerBlock *hcl.Block) map[string]interface{} {
...
func (p *HCLLoader) countReferences(block *hcl.Block) *countExpression {
...
func (p *HCLLoader) marshalBlock(block *hcl.Block, jsonValues map[string]interface{}) {
	for _, b := range block.Children() {
		...
		if v, ok := jsonValues[key]; ok {
			...
			if slice, ok := v.([]interface{}); ok {
				jsonValues[key] = append(slice, childValues)
			} else {
