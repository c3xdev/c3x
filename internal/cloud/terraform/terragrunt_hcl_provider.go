...
import (
	...
	gruntinfo "github.com/c3xdev/c3x/internal/hclstack/cli/commands/info"
	grunttf "github.com/c3xdev/c3x/internal/hclstack/cli/commands/tf"
	"github.com/c3xdev/c3x/internal/hclstack/codegen"
	tgconfig "github.com/c3xdev/c3x/internal/hclstack/config"
	depgraph "github.com/c3xdev/c3x/internal/hclstack/depgraph"
	"github.com/c3xdev/c3x/internal/hclstack/errtrace"
	tgoptions "github.com/c3xdev/c3x/internal/hclstack/options"
	tgterraform "github.com/c3xdev/c3x/internal/hclstack/terraform"
	...
	"github.com/c3xdev/c3x/internal/iac/hcl/mock"
	"github.com/c3xdev/c3x/internal/iac/hcl/modules"
	"github.com/c3xdev/c3x/internal/metrics"
	"github.com/c3xdev/c3x/internal/settings"
	infSync "github.com/c3xdev/c3x/internal/sync"
...
var (
	...
	// because the call to util.CopyFolderContents in grunttf.DownloadTerraformSource seems to be mucking
...
func NewTerragruntHCLProvider(rootPath hcl.RootPath, ctx *settings.ProjectSession) engine.Vendor {
	...
	fetcher := modules.NewPackageFetcher(remoteCache, logger, modules.WithGetters(map[string]getter.Getter{
		"tfr": &tgterraform.RegistryGetter{
			...
			ProxyURL:        os.Getenv("C3X_REGISTRY_PROXY"),
		},
		"file": &grunttf.FileCopyGetter{},
...
func getEnvVars(ctx *settings.ProjectSession) map[string]string {
	environment := os.Environ()
	environmentMap := make(map[string]string)

	var filterSafe bool
	safe := make(map[string]struct{})
	if v, ok := os.LookupEnv("C3X_SAFE_ENVS"); ok {
		filterSafe = true
...
func (p *TerragruntHCLProvider) Context() *settings.ProjectSession { return p.ctx }
...
func (p *TerragruntHCLProvider) ProjectName() string {
	...
	return settings.CleanProjectName(p.RelativePath())
}
...
func (p *TerragruntHCLProvider) LoadResources(usage engine.ConsumptionMap) ([]*engine.Workspace, error) {
	...
	var allProjects []*engine.Workspace
...
func (p *TerragruntHCLProvider) newErroredProject(di *terragruntWorkingDirInfo) *engine.Workspace {
	projectPath := di.configDir
	if absPath, err := filepath.Abs(p.ctx.ProjectConfig.Path); err == nil {
		if relProjectPath, err := filepath.Rel(absPath, projectPath); err == nil {
			projectPath = filepath.Join(p.ctx.ProjectConfig.Path, relProjectPath)
	...
	if di.error != nil {
		metadata.AddError(engine.NewDiagTerragruntEvaluationFailure(di.error))
	}

	project := engine.NewProject(p.generateProjectName(metadata), metadata)
	project.DisplayName = p.ProjectName()

	return project
}

func (p *TerragruntHCLProvider) generateProjectName(metadata *engine.WorkspaceMeta) string {
...
func (p *TerragruntHCLProvider) newProjectMetadata(projectPath string, originalMetadata *engine.WorkspaceMeta) *engine.WorkspaceMeta {
	metadata := engine.DetectProjectMetadata(projectPath)
...
func (p *TerragruntHCLProvider) prepWorkingDirs() ([]*terragruntWorkingDirInfo, error) {
	...
	terragruntCacheDir := filepath.Join(settings.C3XDir, ".terragrunt-cache")
	...
	terragruntOptions := &tgoptions.TerragruntOptions{
		...
		TerraformCliArgs:           []string{gruntinfo.CommandName},
		Env:                        p.env,
	...
	if err != nil {
		return nil, clierror.NewCLIError(
			fmt.Errorf(
				...
				err.Error(),
				fmt.Sprintf("For a list of known issues and workarounds, see: %s", ui.LinkString("https://c3x.dev/docs/features/terragrunt/")),
			),
...
func (p *TerragruntHCLProvider) runTerragrunt(opts *tgoptions.TerragruntOptions) (info *terragruntWorkingDirInfo) {
	...
	h, err := NewHCLProvider(
		settings.NewProjectContext(p.ctx.RunContext, &pconfig, logCtx),
		hcl.RootPath{
...
// downloadSource overrides the Terragrunt download source functionality to use the C3X packageFetcher
...
func loadSourceOnce(sourceURL string, opts *tgoptions.TerragruntOptions, terragruntConfig *tgconfig.TerragruntConfig, packageFetcher *modules.PackageFetcher, logger zerolog.Logger) (string, error) {
	...
	sparseCheckoutEnabled := os.Getenv("C3X_SPARSE_CHECKOUT") == "true"

	// If sparse checkout is enabled add the subdir to the Source URL as a query param
	// so modsource only downloads the required directory.
	...
	if failedHttpsDownload {
		_, err = grunttf.DownloadTerraformSource(sourceURL, opts, terragruntConfig)
		if err != nil {
			return "", err
		}
	}
...
func forceHttpsDownload(sourceURL string, opts *tgoptions.TerragruntOptions, terragruntConfig *tgconfig.TerragruntConfig) bool {
	...
	_, err = grunttf.DownloadTerraformSource(newUrl.String(), opts, terragruntConfig)
	return err == nil
}
...
func (p *TerragruntHCLProvider) fetchModuleOutputs(opts *tgoptions.TerragruntOptions) (cty.Value, error) {
	...
	if p.stack != nil {
		var mod *depgraph.TerraformModule
...
func createStackForTerragruntConfigPaths(path string, terragruntConfigPaths []string, terragruntOptions *tgoptions.TerragruntOptions, howThesePathsWereFound string) (*depgraph.Stack, error) {
	if len(terragruntConfigPaths) == 0 {
		return nil, errtrace.WithStackTrace(depgraph.NoTerraformModulesFound)
	}

	modules, err := depgraph.ResolveTerraformModules(terragruntConfigPaths, terragruntOptions, nil, howThesePathsWereFound)
	if err != nil {
		return nil, err
	}

	stack := &depgraph.Stack{Path: path, Modules: modules}
