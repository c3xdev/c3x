...
func NewAutoscalingGroup(d *engine.ResourceSpec) engine.CatalogItem {
	...
	if !d.IsEmpty("desired_capacity") {
		...
		if instanceCount == 0 {
			logging.Logger.Debug().Msgf("Using instance count 1 for %s since no desired_capacity or non-zero min_size is set. To override this set the instance_count attribute for this resource in the C3X usage file.", a.Address)
			instanceCount = 1
		}
	}
...
func newLaunchConfiguration(d *engine.ResourceSpec, region string, instanceCount int64) *aws.LaunchConfiguration {
...
func newLaunchTemplate(d *engine.ResourceSpec, region string, instanceCount, onDemandBaseCount, onDemandPercentageAboveBaseCount int64) *aws.LaunchTemplate {
...
func newMixedInstancesLaunchTemplate(d *engine.ResourceSpec, region string, capacity int64, mixedInstancePolicyData gjson.Result) *aws.LaunchTemplate {
