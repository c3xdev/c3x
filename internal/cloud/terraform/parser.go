...
import (
	...
	"github.com/c3xdev/c3x/internal/iac/hcl"
	"github.com/c3xdev/c3x/internal/logging"
	"github.com/c3xdev/c3x/internal/cloud/terraform/aws"
	"github.com/c3xdev/c3x/internal/cloud/terraform/azure"
	"github.com/c3xdev/c3x/internal/cloud/terraform/google"
	"github.com/c3xdev/c3x/internal/engine"
)

// These show differently in the plan JSON for Terraform 0.12 and 0.13.
var c3xProviderNames = []string{"c3x", "registry.terraform.io/c3x/c3x"}
...
func (p *Parser) createParsedResource(d *engine.ResourceSpec, u *engine.ConsumptionProfile) parsedResource {
	...
	if registryItem, ok := (*ResourceRegistryMap)[d.Type]; ok {
		if registryItem.NoPrice {
			...
			return parsedResource{
				PartialResource: engine.NewPartialResource(d, resource, nil, registryItem.CloudResourceIDFunc(d)),
				ResourceData:    d,
			}
		...
		if registryItem.CoreRFunc != nil {
			...
			if coreRes != nil {
				return parsedResource{
					PartialResource: engine.NewPartialResource(d, nil, coreRes, registryItem.CloudResourceIDFunc(d)),
					ResourceData:    d,
				}
			}
		} else {
			...
			if res != nil {
				...
				return parsedResource{
					PartialResource: engine.NewPartialResource(d, res, nil, registryItem.CloudResourceIDFunc(d)),
					ResourceData:    d,
				}
			}
		}
...
func (p *Parser) parseJSONResources(parsePrior bool, baseResources []parsedResource, usage engine.ConsumptionMap, confLoader *ConfLoader, parsed, providerConf, vars gjson.Result) []parsedResource {
...
func (p *Parser) setRegion(confLoader *ConfLoader, d *engine.ResourceSpec, providerConf gjson.Result, vars gjson.Result) {
	region := p.getRegion(confLoader, d, providerConf, vars)
	d.RawValues = engine.AddRawValue(d.RawValues, "region", region)
	d.Region = region
...
func (p *Parser) populateUsageData(resData map[string]*engine.ResourceSpec, usage engine.ConsumptionMap) {
	for _, d := range resData {
...
func newParsedPlanConfiguration(pastResources, currentResources []parsedResource, metadatas []engine.VendorMeta, remoteModuleCalls []string) *ParsedPlanConfiguration {
	ppc := ParsedPlanConfiguration{
		PastResources:        make([]*engine.UnpricedEntry, 0, len(pastResources)),
		PastResourceDatas:    make([]*engine.ResourceSpec, 0, len(pastResources)),
		CurrentResources:     make([]*engine.UnpricedEntry, 0, len(currentResources)),
		CurrentResourceDatas: make([]*engine.ResourceSpec, 0, len(currentResources)),
...
func (p *Parser) parseJSON(j []byte, usage engine.ConsumptionMap) (*ParsedPlanConfiguration, error) {
	...
	constraints := parsed.Get("c3x_provider_constraints")
...
func parseProviderConfig(providerConf gjson.Result) []engine.VendorMeta {
	...
	metadatas := make([]engine.VendorMeta, 0, len(keys))

	for _, k := range keys {
		conf := confMap[k]
		md := engine.VendorMeta{
			// Perf/memory leak: Copy gjson string slices that may be returned so we don't prevent
			// the entire underlying parsed json from being garbage collected.
			Name:      strings.Clone(conf.Get("name").String()),
			Filename:  strings.Clone(conf.Get("c3x_metadata.filename").String()),
			StartLine: conf.Get("c3x_metadata.start_line").Int(),
			EndLine:   conf.Get("c3x_metadata.end_line").Int(),
		}

		unknownKeys := conf.Get("c3x_metadata.attributes_with_unknown_keys").Array()
		for _, unknownKey := range unknownKeys {
			...
			if vars.IsArray() {
				...
				md.AttributesWithUnknownKeys = append(md.AttributesWithUnknownKeys, engine.AttributeWithUnknownKeys{
...
func (p *Parser) loadUsageFileResources(u engine.ConsumptionMap) []parsedResource {
	resources := make([]parsedResource, 0)

	for k, v := range u.Data() {
		for _, t := range GetUsageOnlyResources() {
			if strings.HasPrefix(k, fmt.Sprintf("%s.", t)) {
				d := engine.NewResourceData(t, "global", k, nil, gjson.Result{})
...
func (p *Parser) parseResourceData(isState bool, confLoader *ConfLoader, providerConf, planVals, vars gjson.Result) map[string]*engine.ResourceSpec {
	resources := make(map[string]*engine.ResourceSpec)
	...
	for _, r := range planVals.Get("resources").Array() {
		...
		data := engine.NewResourceData(strings.Clone(t), strings.Clone(provider), strings.Clone(addr), nil, v)

		// Perf/memory leak: Copy gjson string slices that may be returned so we don't prevent
		// the entire underlying parsed json from being garbage collected.
		data.Metadata = gjson.ParseBytes([]byte(r.Get("c3x_metadata").Raw)).Map()
...
func (p *Parser) getRegion(confLoader *ConfLoader, d *engine.ResourceSpec, providerConf gjson.Result, vars gjson.Result) string {
...
func overrideRegion(d *engine.ResourceSpec, config *settings.Settings) string {
	region := ""
	providerPrefix := getProviderPrefix(d.Type)
...
func resourceRegion(d *engine.ResourceSpec) string {
	providerPrefix := getProviderPrefix(d.Type)
...
func providerRegion(d *engine.ResourceSpec, providerConf gjson.Result, vars gjson.Result, resConf gjson.Result) string {
	var region string
...
func (p *Parser) stripDataResources(resData map[string]*engine.ResourceSpec) {
...
func (p *Parser) parseReferences(resData map[string]*engine.ResourceSpec, confLoader *ConfLoader) {
	// Create a map of id -> resource data so we can lookup references
	idMap := make(map[string][]*engine.ResourceSpec)
		...
		if f := ResourceRegistryMap.GetDefaultRefIDFunc(d.Type); f != nil {
			for _, defaultID := range f(d) {
				if _, ok := idMap[defaultID]; !ok {
					idMap[defaultID] = []*engine.ResourceSpec{}
				}
		...
		if f := ResourceRegistryMap.GetCustomRefIDFunc(d.Type); f != nil {
			for _, customID := range f(d) {
				if _, ok := idMap[customID]; !ok {
					idMap[customID] = []*engine.ResourceSpec{}
				}
...
func (p *Parser) parseConfReferences(resData map[string]*engine.ResourceSpec, confLoader *ConfLoader, d *engine.ResourceSpec, attr string, registryMap *RegistryItemMap) bool {
	...
	for _, ref := range refs {
		...
		var refData *engine.ResourceSpec
...
func (p *Parser) parseTags(data map[string]*engine.ResourceSpec, confLoader *ConfLoader, providerConf gjson.Result) {
	...
	for _, resourceData := range data {
		...
		if conf := providerConf.Get(gjsonEscape(parseProviderKey(resConf))); conf.Exists() {
			if metadata := conf.Get("c3x_metadata"); metadata.Exists() {
...
func (p *Parser) getTagPropagationInfo(resource *engine.ResourceSpec) *engine.TagPropagation {
	if expected, ok := aws.ExpectedPropagations[resource.Type]; ok {
		propagateTags := resource.GetStringOrDefault(expected.Attribute, "")
		propagation := &engine.TagPropagation{
			From:      &propagateTags,
			To:        expected.To,
...
func isC3XResource(res *engine.ResourceSpec) bool {
	for _, p := range c3xProviderNames {
...
func addressCountIndex(addr string) int {
	...
	if len(m) > 0 {
		i, _ := strconv.Atoi(m[1]) // safe: regex guarantees digits
...
func parseKnownModuleRefs(resData map[string]*engine.ResourceSpec, confLoader *ConfLoader) {
	knownRefs := []struct {
...
func fixKnownModuleRefIssues(resData map[string]*engine.ResourceSpec) {
	knownRefs := []struct {
