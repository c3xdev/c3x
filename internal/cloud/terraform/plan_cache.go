...
func WritePlanCache(p *DirProvider, planJSON []byte) {
	...
	// create the .c3x dir if it doesn't already exist
	if _, err := os.Stat(cacheDir); err != nil {
		if os.IsNotExist(err) {
			err := os.MkdirAll(cacheDir, 0700)
			if err != nil {
				logging.Logger.Debug().Msgf("Couldn't create %v directory: %v", settings.C3XDir, err)
				return
			}
		}
	}
...
func calcCacheDir(p *DirProvider) string {
	...
	if dataDir != (path.Join(p.Path, ".terraform")) {
		...
		return path.Join(dataDir, settings.C3XDir)
	}

	return path.Join(p.Path, settings.C3XDir)
}
