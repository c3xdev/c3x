...
func getLogAnalyticsWorkspaceRegistryItem() *engine.CatalogEntry {
	refs := []string{
		"resource_group_name",
...
func newLogAnalyticsWorkspace(d *engine.ResourceSpec) engine.CatalogItem {
	region := d.Region
	sku := "PerGB2018"
	if !d.IsEmpty("sku") {
...
func logAnalyticsWorkspaceDetectSentinel(d *engine.ResourceSpec) bool {
