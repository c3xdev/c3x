...
func NewAzureRMCDNEndpoint(d *engine.ResourceSpec, u *engine.ConsumptionProfile) *engine.Estimate {
	...
	sku := ""
	var profile *engine.ResourceSpec
...
func cdnOutboundDataCostComponents(region, sku string, u *engine.ConsumptionProfile) []*engine.LineItem {
...
func cdnAccelerationDataTransfersCostComponents(region, sku string, d *engine.ResourceSpec, u *engine.ConsumptionProfile) []*engine.LineItem {
	costComponents := []*engine.LineItem{}
...
func cdnCostComponent(name, unit, region, productName, skuName, meterName, startUsage string, quantity *decimal.Decimal) *engine.LineItem {
