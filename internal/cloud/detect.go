...
import (
	...
	"github.com/c3xdev/c3x/internal/cloud/cloudformation"
...
func Detect(ctx *settings.Session, project *settings.Project, includePastResources bool) (*DetectionOutput, error) {
	...
	projectContext := settings.NewProjectContext(ctx, project, nil)
	...
	case ProjectTypeTerraformPlanJSON:
		return &DetectionOutput{Providers: []engine.Vendor{terraform.NewPlanJSONProvider(projectContext, includePastResources)}, RootModules: 1}, nil
	case ProjectTypeTerraformPlanBinary:
		return &DetectionOutput{Providers: []engine.Vendor{terraform.NewPlanProvider(projectContext, includePastResources)}, RootModules: 1}, nil
	case ProjectTypeTerraformCLI:
		return &DetectionOutput{Providers: []engine.Vendor{terraform.NewDirProvider(projectContext, includePastResources)}, RootModules: 1}, nil
	case ProjectTypeTerragruntCLI:
		return &DetectionOutput{Providers: []engine.Vendor{terraform.NewTerragruntProvider(projectContext, includePastResources)}, RootModules: 1}, nil
	case ProjectTypeTerraformStateJSON:
		return &DetectionOutput{Providers: []engine.Vendor{terraform.NewStateJSONProvider(projectContext, includePastResources)}, RootModules: 1}, nil
	case ProjectTypeCloudFormation:
		return &DetectionOutput{Providers: []engine.Vendor{cloudformation.NewTemplateProvider(projectContext, includePastResources)}, RootModules: 1}, nil
	}
	...
	var autoProviders []engine.Vendor
	for _, rootPath := range rootPaths {
		detectedProjectContext := settings.NewProjectContext(ctx, project, nil)
...
// these manually.
func configFileRootToProvider(rootPath hcl.RootPath, options []hcl.Option, projectContext *settings.ProjectSession, pl *hcl.ProjectLocator) *terraform.HCLLoader {
...
func autodetectedRootToProviders(projectContext *settings.ProjectSession, rootPath hcl.RootPath, options ...hcl.Option) []engine.Vendor {
	var providers []engine.Vendor
...
func isTerragruntDir(path string) bool {
	if val, ok := os.LookupEnv("TERRAGRUNT_CONFIG"); ok {
		if filepath.IsAbs(val) {
			return settings.FileExists(val)
		}
		return settings.FileExists(filepath.Join(path, val))
	}

	return settings.FileExists(filepath.Join(path, "terragrunt.hcl")) || settings.FileExists(filepath.Join(path, "terragrunt.hcl.json"))
}
	...
	if maxDepth > 0 {
		...
		if err == nil {
			for _, entry := range entries {
				name := entry.Name()
				if entry.IsDir() && name != settings.C3XDir && name != ".terraform" {
