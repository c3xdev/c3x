...
func (g *GitGetter) Get(ctx context.Context, dst string, u *url.URL) error {
	...
	if sshKey != "" {
		...
		fh, err := os.CreateTemp("", "c3x-fetch")
		if err != nil {
			return err
		}
		sshKeyFile = fh.Name()
...
func (g *GitGetter) update(ctx context.Context, dst, sshKeyFile string, u *url.URL, ref string, depth int) error {
	...
	// Add the git remote
	cmd = exec.CommandContext(ctx, "git", "remote", "add", "origin", "--", u.String()) //nolint:gosec // git commands with controlled arguments
	cmd.Dir = dst
	err = getRunCommand(cmd)
	if err != nil {
		return err
	...
	if depth > 0 {
		cmd = exec.CommandContext(ctx, "git", "pull", "origin", "--depth", strconv.Itoa(depth), "--ff-only", "--", ref) //nolint:gosec // git commands with controlled arguments
	} else {
		cmd = exec.CommandContext(ctx, "git", "pull", "origin", "--ff-only", "--", ref) //nolint:gosec // git commands with controlled arguments
	}
...
func findRemoteDefaultBranch(ctx context.Context, u *url.URL) string {
	...
	cmd := exec.CommandContext(ctx, "git", "ls-remote", "--symref", "--", u.String(), "HEAD") //nolint:gosec // git commands with controlled arguments
