...
func TestFileGetter_Get_Directory(t *testing.T) {
	...
	srcDir := t.TempDir()
	_ = os.WriteFile(filepath.Join(srcDir, "file1.tf"), []byte("resource {}"), 0600)
	_ = os.WriteFile(filepath.Join(srcDir, "file2.tf"), []byte("variable {}"), 0600)
...
func TestGet_LocalFile(t *testing.T) {
	...
	srcDir := t.TempDir()
	_ = os.WriteFile(filepath.Join(srcDir, "main.tf"), []byte("resource {}"), 0600)
