...
import (
	...
	"bytes"
	"context"
	"crypto/md5"  //nolint:gosec // required for checksum verification
	"crypto/sha1" //nolint:gosec // required for checksum verification
	"crypto/sha256"
	"crypto/sha512"
	"encoding/hex"
	"fmt"
...
// ex:
//
//	http://example.com/terraform?checksum=<checksumValue>
//	http://example.com/terraform?checksum=<checksumType>:<checksumValue>
//	http://example.com/terraform?checksum=file:<checksum_url>
//
...
func newChecksumFromType(checksumType, checksumValue, filename string) (*FileChecksum, error) {
	...
	switch c.Type {
	case "md5":
		c.Hash = md5.New() //nolint:gosec // required for checksum verification
	case "sha1":
		c.Hash = sha1.New() //nolint:gosec // required for checksum verification
	case "sha256":
		c.Hash = sha256.New()
	case "sha512":
		c.Hash = sha512.New()
