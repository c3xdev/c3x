...
func untar(input io.Reader, dst, src string, dir bool, umask os.FileMode, fileSizeLimit int64, filesLimit int) error {
	...
	for {
		...
		if dir {
			...
			path = filepath.Join(path, hdr.Name) //nolint:gosec // path traversal check exists upstream
		}
	...
	for _, dirHdr := range dirHdrs {
		path := filepath.Join(dst, dirHdr.Name) //nolint:gosec // path traversal check exists upstream
