...
type AddRunResponse struct {
	...
	GovernanceFailures render.GovernanceFailures `json:"governanceFailures"`
...
type projectResultInput struct {
	...
	ProjectMetadata *engine.WorkspaceMeta `json:"projectMetadata"`
	PastBreakdown   *render.Breakdown     `json:"pastBreakdown"`
	Breakdown       *render.Breakdown     `json:"breakdown"`
	Diff            *render.Breakdown     `json:"diff"`
	Summary         *render.Summary       `json:"summary"`
}

func NewDashboardAPIClient(ctx *settings.Session) *DashboardAPIClient {
...
func newRunInput(ctx *settings.Session, out render.Report) (*runInput, error) {
...
func (c *DashboardAPIClient) SavePostedPrComment(ctx *settings.Session, runId, comment string) error {
...
func (c *DashboardAPIClient) AddRun(ctx *settings.Session, out render.Report) (AddRunResponse, error) {
	...
	if len(results) > 0 {
		...
		successMsg := fmt.Sprintf("Estimate uploaded to %sC3X Cloud", orgMsg)
...
func outputGovernanceMessages(ctx *settings.Session, msg string) {
