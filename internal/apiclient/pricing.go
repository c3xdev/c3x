...
func GetPricingAPIClient(ctx *settings.Session) *PricingAPIClient {
	pricingMu.Lock()
...
func NewPricingAPIClient(ctx *settings.Session) *PricingAPIClient {
	if ctx == nil {
		return nil
	}
	...
	c := &PricingAPIClient{
		...
		EventsDisabled: ctx.Config.EventsDisabled || ctx.Config.OfflineMode,
		OfflineMode:    ctx.Config.OfflineMode,
	}

	return c
}

func initCache(ctx *settings.Session, c *PricingAPIClient) {
	if ctx.Config.PricingCacheDisabled {
		return
	}

	cacheFile := filepath.Join(ctx.Config.CachePath(), settings.C3XDir, "pricing.gob")
...
func newCache(ctx *settings.Session) *lru.TwoQueueCache[uint64, cacheValue] {
	objectLimit := 1000
...
func loadCacheFromFile(ctx *settings.Session, cacheFile string) *lru.TwoQueueCache[uint64, cacheValue] {
...
func (c *PricingAPIClient) FlushCache() error {
	...
	f, err := os.OpenFile(c.cacheFile, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0600)
	if err != nil {
		return err
	}
	defer f.Close()
...
func (c *PricingAPIClient) buildQuery(product *engine.ProductSelector, price *engine.RateSelector, currency string) GraphQLQuery {
	if currency == "" {
		currency = "USD"
	}
...
func (c *PricingAPIClient) BatchRequests(resources []*engine.Estimate, batchSize int, currency string) []BatchRequest {
...
func (c *PricingAPIClient) PerformRequest(req BatchRequest) ([]PriceQueryResult, error) {
	...
	var resultsFromServer []gjson.Result
	var queryErr error
	if c.OfflineMode {
		resultsFromServer, queryErr = c.doOfflineQueries(rawQueries)
	} else {
		resultsFromServer, queryErr = c.DoQueries(rawQueries)
	}
	if queryErr != nil {
		return []PriceQueryResult{}, queryErr
	}
...
// doOfflineQueries handles pricing queries using the local SQLite database.
// It opens the local pricing DB, queries for each product, and returns results
// in the same format as the remote GraphQL API.
func (c *PricingAPIClient) doOfflineQueries(queries []GraphQLQuery) ([]gjson.Result, error) {
	store, err := localpricing.Open(localpricing.DefaultPath())
	if err != nil {
		return nil, fmt.Errorf("offline mode requires a local pricing database, run 'c3x pricing sync' first: %w", err)
	}
	defer store.Close()
	...
	for i, q := range queries {
		vars := q.Variables
		if vars == nil {
			results[i] = gjson.Parse(`{"data":{"products":[]}}`)
			continue
		}
		...
		// Variables may contain *engine.ProductSelector (from buildQuery)
		// or map[string]interface{} (from JSON deserialization)
		switch pf := vars["productFilter"].(type) {
		case *engine.ProductSelector:
			if pf.VendorName != nil {
				vendor = *pf.VendorName
			}
			if pf.Service != nil {
				service = *pf.Service
			}
			...
			for _, af := range pf.AttributeFilters {
				if af.Key != "" && af.Value != nil {
					attrFilters[af.Key] = *af.Value
				}
			}
		case map[string]interface{}:
			if v, ok := pf["vendorName"].(string); ok {
				vendor = v
			}
			if v, ok := pf["service"].(string); ok {
				service = v
			}
			if v, ok := pf["region"].(string); ok {
				region = v
			}
			if filters, ok := pf["attributeFilters"].([]interface{}); ok {
		...
		result, err := store.LookupPrice(vendor, service, region, attrFilters, "")
		if err != nil {
			results[i] = gjson.Parse(`{"data":{"products":[]}}`)
			continue
		}
