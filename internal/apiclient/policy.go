...
// NewPolicyAPIClient retrieves resource allow-list info from C3X Cloud and returns a new policy client
func NewPolicyAPIClient(ctx *settings.Session) (*PolicyAPIClient, error) {
...
// UploadPolicyData sends a filtered set of a project's resource information to C3X Cloud and
// potentially adds PolicySha and PastPolicySha to the project's metadata.
func (c *PolicyAPIClient) UploadPolicyData(project *engine.Workspace, rds, pastRds []*engine.ResourceSpec) error {
	if project.Metadata == nil {
		project.Metadata = &engine.WorkspaceMeta{}
	}
...
type policy2Resource struct {
	...
	Metadata                                policy2C3XMetadata `json:"c3xMetadata"`
...
type policy2C3XMetadata struct {
	Calls          []policy2C3XMetadataCall `json:"calls"`
...
type policy2C3XMetadataCall struct {
	Filename  string `json:"filename"`
...
func (c *PolicyAPIClient) filterResources(rds []*engine.ResourceSpec) []policy2Resource {
...
func filterResource(rd *engine.ResourceSpec, al allowList) policy2Resource {
	...
	if rd.TagPropagation != nil && rd.TagPropagation.Tags != nil {
		tags := make([]policy2Tag, 0, len(*rd.TagPropagation.Tags))
	...
	mdCalls := make([]policy2C3XMetadataCall, 0, len(calls))
	for _, c := range calls {
		mdCalls = append(mdCalls, policy2C3XMetadataCall{
...
func calcChecksum(rd *engine.ResourceSpec) string {
	h := sha256.New()
