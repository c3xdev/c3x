...
var (
	StubMetadata = Metadata{
		Remote: urlStringToRemote("https://github.com/c3xdev/c3x"),
		Branch: Branch{
			Name: "stub-branch",
		},
		Commit: Commit{
...
func buildEnvProvidedMetadata() Metadata {
	remote := urlStringToRemote(getEnv("C3X_VCS_REPOSITORY_URL"))
	m := Metadata{
		Remote: remote,
		Branch: Branch{
			Name: getEnv("C3X_VCS_BRANCH"),
		},
		Commit:      buildDefaultCommit(),
		PullRequest: buildDefaultPR(),
		Pipeline:    &Pipeline{ID: getEnv("C3X_VCS_PIPELINE_RUN_ID")},
	}
...
func buildDefaultPR() *PullRequest {
	if !keysSet(
		"C3X_VCS_PULL_REQUEST_ID",
		"C3X_VCS_PROVIDER",
		"C3X_VCS_PULL_REQUEST_TITLE",
		"C3X_VCS_PULL_REQUEST_AUTHOR",
		"C3X_VCS_PULL_REQUEST_LABELS",
		"C3X_VCS_BRANCH",
		"C3X_VCS_BASE_BRANCH",
		"C3X_VCS_PULL_REQUEST_URL",
	) {
		return nil
	}

	provider := strings.ToLower(getEnv("C3X_VCS_PROVIDER"))
	if _, ok := allowedProviders[provider]; !ok && provider != "" {
		logging.Logger.Warn().Msgf("provided value for C3X_VCS_PROVIDER '%s' is not valid. Setting vcsProvider to an empty string", provider)
		provider = ""
	}

	prURL := getEnv("C3X_VCS_PULL_REQUEST_URL")
	prID := getEnv("C3X_VCS_PULL_REQUEST_ID")
	if prURL != "" && prID == "" {
	...
	return &PullRequest{
		...
		Title:        getEnv("C3X_VCS_PULL_REQUEST_TITLE"),
		Author:       getEnv("C3X_VCS_PULL_REQUEST_AUTHOR"),
		Labels:       getEnvList("C3X_VCS_PULL_REQUEST_LABELS"),
		SourceBranch: getEnv("C3X_VCS_BRANCH"),
		BaseBranch:   getEnv("C3X_VCS_BASE_BRANCH"),
		URL:          prURL,
	}
}
...
func buildDefaultCommit() Commit {
	return Commit{
		SHA:         getEnv("C3X_VCS_COMMIT_SHA"),
		AuthorName:  getEnv("C3X_VCS_COMMIT_AUTHOR_NAME"),
		AuthorEmail: getEnv("C3X_VCS_COMMIT_AUTHOR_EMAIL"),
		Time:        envToTime("C3X_VCS_COMMIT_TIMESTAMP"),
		Message:     getEnv("C3X_VCS_COMMIT_MESSAGE"),
	}
}
...
func envToTime(key string) time.Time {
	...
	if err != nil {
		logging.Logger.Debug().Err(err).Msgf("could not parse 'C3X_COMMIT_TIMESTAMP' value '%s' as int64 timestamp", env)
		return time.Time{}
	}
...
func (f *metadataFetcher) isTest() bool {
	...
	return getEnv("C3X_ENV") == "test" || strings.HasSuffix(os.Args[0], ".test")
}
...
func (f *metadataFetcher) Get(path string, gitDiffTarget *string) (m Metadata, err error) {
	...
	if f.isTest() {
		logging.Logger.Debug().Msg("returning stub metadata as C3X is running in test mode")
...
func (f *metadataFetcher) getGithubMetadata(path string, gitDiffTarget *string) (Metadata, error) {
	...
	if m.Branch.Name == "HEAD" {
		...
		// Sanitize attacker-controlled values to prevent path traversal.
		// Repository name and branch ref come from the GitHub webhook event.
		repoName := filepath.Base(gjson.GetBytes(event, "repository.name").String())
		headRef = filepath.Base(headRef)
		clonePath := filepath.Join(os.TempDir(), fmt.Sprintf("c3x-%s-%s", repoName, headRef))
		unlock := f.mu.Lock(clonePath)

		// if the clone path already exists then let's just do a plain open. We might hit this
		// if the user is running multiple C3X commands on the head commit. We don't want
		// to clone each time.
...
// This is used to aggregate C3X metadata across commands used in the same pipeline.
