...
func CheckForUpdate(ctx *settings.Session) (*Info, error) {
	...
	var cmd string
	if isBrew {
		cmd = "$ brew upgrade c3x"
	} else {
		cmd = "Go to https://www.c3x.dev/docs/update for instructions"
		if runtime.GOOS == "linux" || runtime.GOOS == "darwin" {
			cmd = "$ curl -fsSL https://raw.githubusercontent.com/c3xdev/c3x/master/scripts/install.sh | sh"
		}
	}
...
func skipUpdateCheck(ctx *settings.Session) bool {
	return ctx.Config.SkipUpdateCheck || settings.IsTest() || settings.IsDev()
}
...
func isBrewInstall() (bool, error) {
	...
	if err != nil {
		return false, errors.Wrap(err, "error finding c3x executable")
	}

	path, err := filepath.EvalSymlinks(exe)
	if err != nil {
		return false, errors.Wrap(err, "error evaluating c3x executable symlink")
	}

	brewPrefixCmd := exec.Command("brew", "--prefix", "c3x")
	...
	if err != nil {
		return false, errors.Wrap(err, "error running 'brew --prefix c3x'")
	}
	...
	brewC3XPath := filepath.Join(brewPrefixPath, "bin", "c3x")

	return path == brewC3XPath, nil
}
...
func getLatestBrewVersion() (string, error) {
	...
	resp, err := http.Get("https://formulae.brew.sh/api/formula/c3x.json")
	if err != nil {
		return "", err
	}
...
func getLatestGitHubVersion() (string, error) {
	...
	resp, err := http.Get("https://api.github.com/repos/c3xdev/c3x/releases/latest")
	if err != nil {
		return "", err
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode != http.StatusOK {
		return "", errors.Errorf("Error getting latest version from GitHub: %s", resp.Status)
	}
...
func checkCachedLatestVersion(ctx *settings.Session) (string, error) {
...
func setCachedLatestVersion(ctx *settings.Session, latestVersion string) error {
