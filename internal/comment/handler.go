...
// nonRetryableStatusCodes are HTTP status codes that indicate the request
// should not be retried because the issue is with the request itself.
var nonRetryableStatusCodes = []string{"401", "403", "404", "422"}

// isRetryableError returns true if the error is likely transient and the
// request should be retried (e.g. 429, 5xx, or network errors).
...
func (h *CommentHandler) CommentWithBehavior(ctx context.Context, behavior, body string, opts *CommentOpts) (PostResult, error) {
	...
	for i := 0; i < maxRetries; i++ {
		...
		if !isRetryableError(err) {
			logging.Logger.Debug().Err(err).Msg("received a non-retryable error posting comment, will not retry")
			return result, err
		}
		...
		select {
		case <-ctx.Done():
			return result, fmt.Errorf("context cancelled while retrying comment post: %w", ctx.Err())
...
func (h *CommentHandler) newPlatformError(err error) error {
	...
	return fmt.Errorf("%s\n%w\n\n%s",
		...
		err,
		"See https://c3x.dev/docs/troubleshooting/#5-posting-comments for help.")
}
