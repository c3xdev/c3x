...
test_shared_int:
	C3X_LOG_LEVEL=warn go test -timeout 30m $(LD_FLAGS) \
...
parser_benchmarks:
	go test -bench=. -benchmem -cpuprofile=cpu.out -memprofile=mem.out -trace=trace.out github.com/c3xdev/c3x/internal/hcl
