...
func fetchExistingRelease(cli *github.Client, tag string) (*github.RepositoryRelease, error) {
	release, _, err := cli.Repositories.GetReleaseByTag(context.Background(), "c3xdev", "c3x", tag)
	if err != nil {
	...
	if newGitSha != "" {
		_, _, err = cli.Git.UpdateRef(context.Background(), "c3xdev", "c3x", &github.Reference{
	...
	for _, asset := range release.Assets {
		_, err = cli.Repositories.DeleteReleaseAsset(context.Background(), "c3xdev", "c3x", asset.GetID())
		if err != nil {
